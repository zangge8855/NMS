...
6. Tune homepage access path, invite-only registration, SMTP, backups, security, storage runtime modes, and the embedded node console in `Settings`.
...
- 只强调 `选类型 -> 复制地址 -> 导入客户端`
- 订阅地址输入框会收成短地址展示，二维码和快捷导入按钮紧挨着显示
