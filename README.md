...
6. Tune homepage access path, optional camouflage landing page, invite-only registration, SMTP, backups, security, storage runtime modes, and the embedded node console in `Settings`.
...
6. 在 `Settings` 里配置首页访问路径、邀请注册、SMTP、备份、安全策略、存储运行模式和节点控制台。
