...
2026-07-13 补充复核：展开侧栏宽度调整为 260px，并将退出区固定为独立底部工具区；服务器、用户、入站和审计在 1024px 等中窄桌面切换为完整卡片布局，服务器与入站在 1580px 以下使用合并信息列，消除展开侧栏后的横向裁切。普通用户订阅中心重新补齐实际生效的导入工作台样式，统一标题、状态、配置切换、长地址、快捷导入、二维码和重置风险区。三套公开伪装模板改为更克制的编辑型杂志排版，修复语言切换占整行、孤立内容卡和通用卡片网格问题，并完成桌面 / 手机、中文 / 英文、浅色 / 深色复核。
