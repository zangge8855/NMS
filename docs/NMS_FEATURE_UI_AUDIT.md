...
| 流量日志审计 | 已完整覆盖 | 审计中心包含操作审计、3x-ui 日志聚合、流量统计趋势、订阅访问日志（含状态分布） |
...
- 跨节点入站聚合列表
- 支持手工拖拽排序，排序结果会持久保存并影响订阅输出顺序
- 批量操作：启用、停用、删除、重置流量、批量加用户
- 入站展开后可直接查看用户流量、剩余额度，并手动删除历史用户
- 入站编辑器：
...
- 冲突扫描与自动修复（跨协议标识冲突）
- 订阅策略弹窗（限制可访问节点/协议）
- 订阅链接与二维码（多 profile：v2rayN/Raw/Native/Reconstructed/Clash YAML/Mihomo YAML/sing-box）
...
- 流量统计：总量、活跃用户、采样点、用户/节点趋势图；用户级流量缺失时可回退到入站总流量
- 订阅访问：PV/UV/状态分布、访问明细分页

补充说明：

- 审计中心里的日志页现在固定用于集中查看各节点的 `3x-ui panel log`
- 不再把宿主机 `syslog` 当作集中审计日志来源
...
- 级别过滤、行数控制、关键词过滤、复制
- 集群模式支持节点选择与聚合显示
- 全局视角默认聚合 3x-ui panel 日志，避免被单节点“系统日志”状态污染
