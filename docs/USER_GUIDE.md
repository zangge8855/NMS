...
5. In Telegram itself, use `/help` or type `/` to discover the available commands from the synchronized command menu.
...
5. 在 Telegram 对话框里输入 `/`，可以直接看到同步后的命令菜单；也可以使用 `/help` 查看完整说明。
