...
function hashTokenSecret(secret: unknown): string {
    return crypto
...
function generatePublicTokenId(): string {
...
class SubscriptionTokenStore {
    ...
    _resolvePublicId(record: any): string {
        const explicit = String(record?.publicId || '').trim();
        if (explicit) return explicit;
        return String(record?.id || '').trim();
    }

    _ensurePublicIds(): boolean {
        let changed = false;
    ...
    _encryptSecret(secret: unknown): string {
    ...
    _decryptSecret(payload: unknown): string {
    ...
    _isRevoked(record: any): boolean {
        return Boolean(record?.revokedAt);
    }

    _isExpired(record: any): boolean {
        if (!record?.expiresAt) return false;
        return new Date(record.expiresAt).getTime() <= Date.now();
    }

    _sanitize(record: any): any {
    ...
    listByEmail(email: string, options: any = {}): any[] {
    ...
    issue(email: string, options: any = {}): any {
        const normalizedEmail = normalizeEmail(email);
        if (!normalizedEmail) {
    ...
    verify(email: string, tokenId: string, tokenSecret: string): { ok: boolean; reason: string; record: any } {
        const normalizedEmail = normalizeEmail(email);
    ...
    touchLastUsed(email: string, tokenId: string): boolean {
        const normalizedEmail = normalizeEmail(email);
        const lookupTokenId = String(tokenId || '').trim();
        const record = this.tokens.find(
            (item) => (
    ...
    revoke(email: string, tokenId: string, reason: string = 'manual-revoke'): any {
        const normalizedEmail = normalizeEmail(email);
        const record = this.tokens.find(
    ...
    revokeAllByEmail(email: string, reason: string = 'manual-revoke-all'): number {
        const normalizedEmail = normalizeEmail(email);
    ...
    reassignEmail(sourceEmail: string, targetEmail: string, options: any = {}): any {
        ...
        const scopedIds = Array.isArray(options.tokenIds) && options.tokenIds.length > 0
            ? new Set<string>(options.tokenIds.map((item: any) => String(item || '').trim()).filter(Boolean))
            : null;

        const movedRecords: any[] = [];
        this.tokens.forEach((record) => {
    ...
    getByEmailAndId(email: string, tokenId: string): any {
        const normalizedEmail = normalizeEmail(email);
        const record = this.tokens.find(
            (item) => item.email === normalizedEmail && item.id === String(tokenId || '').trim()
        );
    ...
    getTokenById(email: string, tokenId: string): any {
        const normalizedEmail = normalizeEmail(email);
    ...
    getTokenByPublicId(tokenId: string): any {
        const lookupTokenId = String(tokenId || '').trim();
    ...
    verifyByTokenId(tokenId: string, tokenSecret: string): { ok: boolean; reason: string; record: any; email: string } {
    ...
    touchLastUsedByTokenId(tokenId: string): boolean {
        const record = this.getTokenByPublicId(tokenId);
        if (!record) return false;
        record.lastUsedAt = new Date().toISOString();
        this._save();
    ...
    getRawTokenById(email: string, tokenId: string): string | null {
    ...
    getFirstActiveToken(email: string): { metadata: any; token: string } | null {
        const normalizedEmail = normalizeEmail(email);
    ...
    getFirstActiveTokenByName(email: string, name: string = ''): { metadata: any; token: string } | null {
        const normalizedEmail = normalizeEmail(email);
    ...
    migrateLegacySigToken(email: string, signature: string, options: any = {}): any {
        const normalizedEmail = normalizeEmail(email);
    ...
    parseToken(rawToken: unknown): { tokenId: string; tokenSecret: string } {
    ...
    normalizeDateInput(value: unknown): string | null {
