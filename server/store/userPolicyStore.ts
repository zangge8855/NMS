...
function toUniqueStringArray(input: unknown = [], mapper: (x: any) => string = (x) => String(x || '').trim()): string[] {
    const result: string[] = [];
...
function sanitizePolicy(input: any = {}): any {
...
class UserPolicyStore {
    ...
    upsert(email: string, policyInput: any = {}, actor: string = 'admin'): any {
        const normalizedEmail = normalizeEmail(email);
        if (!normalizedEmail) {
    ...
    reassignEmail(sourceEmail: string, targetEmail: string, actor: string = 'admin'): any {
    ...
    removeServerId(serverId: string, actor: string = 'system'): number {
        ...
        for (const [email, rawRecord] of Object.entries(this.policies)) {
            ...
            const nextAllowed = inAllowed
                ? sanitized.allowedServerIds.filter((item: string) => item !== targetId)
                : sanitized.allowedServerIds;
            const nextBlocked = inBlocked
                ? sanitized.blockedServerIds.filter((item: string) => item !== targetId)
