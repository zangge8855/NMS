...
class ServerStore {
    servers: any[];
    decryptWarningCache: Set<string>;
    ...
    _isEncryptedPassword(value: unknown): boolean {
        return typeof value === 'string' && value.startsWith(`${PASSWORD_ENC_PREFIX}:`);
    }

    _encryptPassword(password: unknown): any {
    ...
    _decryptPasswordDetailed(password: unknown): { value: string; status: 'missing' | 'plaintext' | 'unreadable' | 'ok' } {
    ...
    _decryptPassword(password: unknown): string {
        return this._decryptPasswordDetailed(password).value;
    }

    _migrateLegacyPasswords(): void {
        let changed = false;
    ...
    _normalizeSshPort(value: unknown, fallback: number = 22): number {
        const parsed = Number.parseInt(String(value), 10);
        if (!Number.isInteger(parsed) || parsed < 1 || parsed > 65535) return fallback;
        return parsed;
    }

    _normalizeSshAuthMethod(value: unknown): string {
        const method = String(value || '').trim().toLowerCase();
        return method === 'key' ? 'key' : 'password';
    }

    _normalizeSshUsername(value: unknown): string {
        const username = String(value || '').trim();
        return username || 'root';
    }

    _normalizeGovernanceGroup(value: unknown): string {
        const text = String(value || '').trim();
        if (!text) return '';
        return text.slice(0, 64);
    }

    _normalizeGovernanceEnvironment(value: unknown): string {
    ...
    _normalizeGovernanceHealth(value: unknown): string {
        const text = String(value || '').trim().toLowerCase();
        if (!text) return 'unknown';
    ...
    _normalizeGovernanceTags(value: unknown): string[] {
        let candidates: any[] = [];
    ...
    _normalizeGovernanceFields(input: any = {}): any {
        return {
    ...
    _hasSshCredentials(server: any): boolean {
        return Boolean(server?.sshPassword || server?.sshPrivateKey);
    }

    _warnDecryptFailure(rawValue: unknown, reason: string): void {
    ...
    _resolveCredentialStatus(rawPassword: unknown, rawApiToken: unknown = ''): 'unreadable' | 'missing' | 'configured' {
    ...
    _normalizeBasePath(basePath: unknown): string {
    ...
    setSession(id: string, cookie: any): void {
    ...
    rotateCredentials(options: any = {}): any {
