...
const TRANSIENT_OFFLINE_REASONS = new Set<string>([
    'dns_error',
    'connect_timeout',
    'connection_refused',
    'network_error',
...
function loadTelemetry(): Record<string, any[]> {
...
function toIsoTime(value: unknown): string {
    const date = new Date((value as any) || Date.now());
...
function normalizePoints(value: unknown, fallback: number = DEFAULT_POINT_COUNT): number {
    const parsed = Number(value);
    if (!Number.isFinite(parsed) || parsed <= 1) return fallback;
    return Math.max(2, Math.min(96, Math.floor(parsed)));
}

function normalizeHours(value: unknown, fallback: number = DEFAULT_WINDOW_HOURS): number {
    const parsed = Number(value);
    if (!Number.isFinite(parsed) || parsed <= 0) return fallback;
    return Math.max(1, Math.min(72, Math.floor(parsed)));
}

function normalizeSample(sample: any = {}): any {
    return {
...
function pruneSamples(list: any[] = [], minTimeMs: number = 0): any[] {
    return list
        .map((item) => normalizeSample(item))
        .filter((item) => Date.parse(item.ts) >= minTimeMs);
}

function sampleStateChanged(previous: any = null, next: any = null): boolean {
...
function isTransientOfflineSample(sample: any = {}): boolean {
    return sample.online !== true && TRANSIENT_OFFLINE_REASONS.has(String(sample.reasonCode || ''));
}

function bucketSamples(list: any[] = [], pointCount: number = DEFAULT_POINT_COUNT): any[] {
...
function createEmptyOverview(server: any = {}): any {
    return {
...
class ServerTelemetryStore {
    samplesByServerId: Record<string, any[]>;
    pendingOfflineByServerId: Map<string, any>;
    ...
    recordSnapshot(snapshot: any = {}): void {
    ...
    recordSample(serverId: string, sample: any): boolean {
    ...
    buildOverviewForServer(server: any = {}, options: any = {}): any {
