...
let trafficWarmLoopTimer: NodeJS.Timeout | null = null;
let trafficWarmLoopInFlight: Promise<any> | null = null;
...
function loadObject(file: string, fallback: any = {}): any {
...
async function saveJsonAsync(file: string, data: any): Promise<void> {
...
function normalizeReferenceDate(value: unknown): Date {
    const date = value instanceof Date ? new Date(value) : new Date((value as any) || Date.now());
    if (Number.isNaN(date.getTime())) return new Date();
    return date;
}

function buildCalendarTrafficWindowRange(windowKey: unknown, reference: Date = new Date()): { fromTs: number; toTs: number; from: string; to: string } | null {
...
function hasPositiveRegisteredTrafficTotals(value: unknown): boolean {
...
function hasPositiveServerTrafficTotals(value: unknown): boolean {
    return normalizeServerTotals(value).some((item) => Number(item.totalBytes || 0) > 0);
}

function resolveTrafficBaselineStatus(meta: any = {}): { currentTotalsAt: string | null; baselineReady: boolean; registeredTotalsReady: boolean; serverTotalsReady: boolean } {
...
function buildTrafficUserDirectoryVersion(users: any[] = []): string {
...
function buildResolvedTrafficUserDirectory(users: any[] = []): { aliasMap: Map<string, any>; totalUsers: number; versionToken: string } {
...
function resolveTrafficUserInfo(email: unknown, directory: any = null): any {
...
function collectStatsCandidates(inbound: any): any[] {
    const candidates = [
        inbound?.clientStats,
...
function resolveClientMergeKeys(client: any = {}): string[] {
...
function extractInboundClients(inbound: any): { clients: any[]; hasClientTraffic: boolean } {
    const settings = parseJsonObjectLike(inbound?.settings, {}) as any;
    ...
    const merged = baseClients.map((client: any) => {
...
function shouldUseInboundTotalFallback(hasClientTraffic: boolean, clientTrafficCaptured: boolean): boolean {
...
const REGISTERED_TRAFFIC_PROTOCOLS = new Set<string>(['vmess', 'vless', 'trojan', 'shadowsocks']);

function createRegisteredTotals(totalUsers: number = 0): any {
    return {
...
function normalizeRegisteredTotals(value: any, fallbackTotalUsers: number = 0): any {
...
function hasRegisteredTotalsSnapshot(meta: any): boolean {
    return !!meta && typeof meta === 'object' && !Array.isArray(meta) && !!meta.registeredTotals;
}

function createServerTotalEntry(entry: any = {}): any {
...
function normalizeServerTotals(value: any): any[] {
...
function hasServerTotalsSnapshot(meta: any): boolean {
    return !!meta && typeof meta === 'object' && !Array.isArray(meta) && Array.isArray(meta.serverTotals);
}

function normalizeSampleKind(value: unknown): string {
    return String(value || '').trim() === SAMPLE_KIND_SERVER_SNAPSHOT
        ? SAMPLE_KIND_SERVER_SNAPSHOT
        : SAMPLE_KIND_CLIENT_DELTA;
}

function resolveInboundTrafficSummary(inbound: any): { upBytes: number; downBytes: number; totalBytes: number } {
...
function summarizeServerTrafficTotals(serverPayloads: any[] = []): any[] {
...
function shouldTrackRegisteredTrafficInbound(inbound: any): boolean {
...
function buildTrafficUserDirectory(users: any[] = []): any {
    return buildResolvedTrafficUserDirectory(users);
}

function summarizeRegisteredTrafficTotals(users: any[] = [], serverPayloads: any[] = []): any {
...
function calculateTrafficDelta(currentValue: unknown, previousValue: unknown): number {
...
async function runWithConcurrency<T, R>(items: T[], worker: (item: T, index: number) => Promise<R>, concurrency: number = 3): Promise<R[]> {
...
function floorToHour(inputDate: Date): Date {
    const date = new Date(inputDate);
    date.setMinutes(0, 0, 0);
    return date;
}

function floorToDay(inputDate: Date): Date {
    const date = new Date(inputDate);
    date.setHours(0, 0, 0, 0);
    return date;
}

function chooseAutoGranularity(fromTs: number, toTs: number): string {
...
class TrafficStatsStore {
    ...
    meta: any;
    collectingPromise: Promise<any> | null;
    stateVersion: number;
    ...
    _prune(): void {
        ...
        const rollupClientSamples: any[] = [];
        const rollupServerSnapshots: any[] = [];
    ...
    _buildTimeRange(options: any = {}, defaults: any = {}): { fromTs: number; toTs: number; from: string; to: string } {
        const now = Date.now();
    ...
    _filterSamples({ fromTs, toTs, email = '', serverId = '', kind = SAMPLE_KIND_CLIENT_DELTA }: any): any[] {
    ...
    _aggregateTrend(samples: any[], granularity: string): any[] {
    ...
    _aggregateInboundBreakdown(samples: any[] = []): { totals: any; items: any[] } {
    ...
    _aggregateSnapshotDeltaTrend(samples: any[], granularity: string, range: any = {}): any[] {
        const bucketMap = new Map<string, any>();
    ...
    _normalizeOverviewRequests(requests: any[] = [], defaults: any = {}): any[] {
    ...
    _buildOverviewBatchCacheKey(requests: any[] = [], userDirectory: any = null): string {
    ...
    _setOverviewCache(cacheKey: string, value: any): void {
    ...
    _computeOverviewBatch(requests: any[] = [], userDirectory: any = null): Record<string, any> {
        ...
        const requestStateList = requestList.map((request) => ({
            ...
            state: {
                ...
                serverEntries: [] as any[],
            },
        }));
        const serverSnapshotBuckets = new Map<string, any>();
        ...
        if (serverSnapshotBuckets.size > 0) {
            ...
            serverSnapshotBuckets.forEach((bucket) => {
                bucket.samples.sort((left: any, right: any) => left.sampleTs - right.sampleTs);
                const perRequestTotals = new Map<string, any>(
                ...
                bucket.samples.forEach((sample: any) => {
                    const deltaUp = calculateTrafficDelta(sample.upBytes, previousSample?.upBytes);
                    const deltaDown = calculateTrafficDelta(sample.downBytes, previousSample?.downBytes);
                    previousSample = sample;
    ...
    getOverviewBatch(requests: any[] = [], options: any = {}): any {
    ...
    async _collectFromServer(serverMeta: any, collectedAtIso: string, options: any = {}): Promise<{ samples: any[]; warning: any; inbounds: any }> {
        ...
        const result: { samples: any[]; warning: any; inbounds: any } = { samples: [], warning: null, inbounds: null };
        try {
    ...
    async collectIfStale(force: boolean = false): Promise<any> {
    ...
    async _collectNow(options: any = {}): Promise<any> {
    ...
    getUserTrend(email: string, options: any = {}): any {
        ...
        const samples = this._filterSamples({
            fromTs: range.fromTs,
            toTs: range.toTs,
        }).filter((item: any) => userInfo.aliases.has(normalizeClientEmail(item.email)));
    ...
    getServerTrend(serverId: string, options: any = {}): any {
        const normalizedServerId = String(serverId || '').trim();
        const range = this._buildTimeRange(options, { defaultDays: 30 });
        const selectedGranularity = String(options.granularity || 'auto').toLowerCase();
        const granularity = selectedGranularity === 'auto'
...
function runTrafficWarmCycle(): Promise<any> | null {
...
function startTrafficStatsWarmLoop(options: any = {}): void {
...
function stopTrafficStatsWarmLoop(): void {
