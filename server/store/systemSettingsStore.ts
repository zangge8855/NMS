...
const GEO_PROVIDERS = new Set<string>(['ip_api', 'ipip_myip']);
const CAMOUFLAGE_TEMPLATES = new Set<string>(['corporate', 'nginx', 'blog']);
...
function compareInboundFallback(a: any, b: any): number {
...
function normalizeInboundOrderMap(input: unknown = {}): Record<string, string[]> {
...
function loadObjectDetailed(file: string): { value: any; error: Error | null } {
    try {
...
function normalizeInt(value: unknown, fallback: number, options: { min?: number; max?: number } = {}): number {
...
function normalizeSiteAccessPath(value: unknown, fallback: string = '/'): string {
...
function isReservedSiteAccessPath(value: string = ''): boolean {
...
function normalizeHttpTemplateUrl(value: unknown, fallback: string = ''): string {
    const text = String(value || '').trim();
    if (!text) return '';
    try {
...
function normalizeCamouflageTemplate(value: unknown, fallback: string = DEFAULT_CAMOUFLAGE_TEMPLATE): string {
    const candidate = String(value || '').trim().toLowerCase();
    return CAMOUFLAGE_TEMPLATES.has(candidate) ? candidate : fallback;
}

function normalizeCamouflageTitle(value: unknown, fallback: string = DEFAULT_CAMOUFLAGE_TITLE): string {
...
function normalizeTelegramChatId(value: unknown, fallback: string = ''): string {
    const text = String(value || '').trim();
    if (!text) return String(fallback || '').trim();
    return text.replace(/\s+/g, '').slice(0, 160);
}

function normalizeAlertMutedUntil(value: unknown, fallback: string = ''): string {
...
function normalizeDailyBackupTime(value: unknown, fallback: string = DEFAULT_DAILY_BACKUP_TIME): string {
    const text = String(value || '').trim();
...
function maskTelegramTokenPreview(value: unknown): string {
...
function mergeSection(base: any, patch: any): any {
    const next = { ...base };
...
function shouldUpgradeLegacyAuditIpGeo(provider: unknown, endpoint: unknown): boolean {
...
export class SystemSettingsStore {
    ...
    _isEncryptedTelegramToken(value: unknown): boolean {
        return typeof value === 'string' && value.startsWith(`${TELEGRAM_TOKEN_ENC_PREFIX}:`);
    }

    _encryptTelegramToken(token: unknown): string {
    ...
    _decryptTelegramTokenDetailed(payload: unknown): { value: string; status: 'missing' | 'plaintext' | 'unreadable' | 'ok' } {
    ...
    _buildPublicTelegram(section: any = {}): any {
    ...
    _normalizeSecurity(input: any = {}, fallback: any): any {
        const security = {
    ...
    _normalizeRegistration(input: any = {}, fallback: any): any {
        return {
            inviteOnlyEnabled: normalizeBoolean(input.inviteOnlyEnabled, fallback.inviteOnlyEnabled),
        };
    }

    _normalizeSite(input: any = {}, fallback: any): any {
    ...
    _normalizeJobs(input: any = {}, fallback: any): any {
        const jobs = {
    ...
    _normalizeAudit(input: any = {}, fallback: any): any {
        return {
            retentionDays: normalizeInt(input.retentionDays, fallback.retentionDays, { min: 1, max: 3650 }),
            maxPageSize: normalizeInt(input.maxPageSize, fallback.maxPageSize, { min: 20, max: 1000 }),
        };
    ...
    _normalizeSubscription(input: any = {}, fallback: any): any {
        return {
    ...
    _normalizeAuditIpGeo(input: any = {}, fallback: any): any {
    ...
    _normalizeTelegram(input: any = {}, fallback: any): any {
    ...
    _normalizeSettings(input: any = {}, fallbackSettings: any = null): any {
    ...
    _normalizeGeofiles(input: any = {}, fallback: any = {}): any {
        const sources = Array.isArray(input?.customSources) ? input.customSources : (fallback?.customSources || []);
        const seen = new Set<string>();
        const cleaned: Array<{ name: string; url: string; type: string }> = [];
    ...
    _assertAllowedPayload(patch: any): void {
        if (!patch || typeof patch !== 'object' || Array.isArray(patch)) {
        ...
        for (const [section, sectionValue] of Object.entries(patch)) {
            ...
            for (const key of Object.keys(sectionValue as any)) {
                if (!allowed.has(key)) {
    ...
    getGeofiles(): any {
        ...
        return {
            customSources: Array.isArray(section.customSources)
                ? section.customSources.map((item: any) => ({ ...item }))
                : [],
        };
    }
    ...
    getTelegram(sectionOverride?: any): any {
    ...
    getInboundOrder(serverId: string = ''): any {
    ...
    sortInboundList(serverId: string, inbounds: any[] = []): any[] {
        ...
        const orderIndex = new Map(order.map((id: string, index: number) => [normalizeId(id), index]));
        ...
        return rows.sort((left, right) => {
            ...
            if (leftKnown && rightKnown) return (leftIndex as number) - (rightIndex as number);
    ...
    setInboundOrder(serverId: string, inboundIds: string[] = []): string[] {
        const normalizedServerId = normalizeId(serverId);
        ...
        const nextInboundOrder: Record<string, string[]> = {
    ...
    setUserOrder(userIds: string[] = []): string[] {
    ...
    setServerOrder(serverIds: string[] = []): string[] {
