...
function buildKey(serverId: unknown, inboundId: unknown, clientIdentifier: unknown): string {
    return [
...
function sanitizeRecord(input: any = {}, actor: string = 'admin'): any {
...
class ClientEntitlementOverrideStore {
    ...
    get(serverId: string, inboundId: string | number, clientIdentifier: string): any {
        const key = buildKey(serverId, inboundId, clientIdentifier);
        const record = this.records[key];
        return record ? { ...record } : null;
    }

    upsert(input: any = {}, actor: string = 'admin'): any {
