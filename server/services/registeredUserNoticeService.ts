...
export function normalizeNoticeScope(value: unknown): string {
    const text = String(value || '').trim().toLowerCase();
    return text === NOTICE_SCOPE.VERIFIED ? NOTICE_SCOPE.VERIFIED : NOTICE_SCOPE.ALL;
}

export function resolveRegisteredUserNoticeRecipients(users: any[] = [], options: any = {}) {
...
export function buildRegisteredUserNoticePreview(payload: any = {}, deps: any = {}) {
...
export async function sendRegisteredUserNoticeCampaign(payload: any = {}, deps: any = {}) {
    const subject = String(payload.subject || '').trim();
    const message = String(payload.message || '').trim();
    const actionUrl = String(payload.actionUrl || '').trim();
    const actionLabel = String(payload.actionLabel || '').trim() || '查看详情';
