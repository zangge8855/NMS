...
function parseJsonTemplate(raw: unknown): any {
    return parseJsonObject(raw);
}

function isUnsupportedPanelRouteError(err: any): boolean {
    const status = Number(err?.response?.status || 0);
    return status === 404 || status === 405;
}

async function requestFirstSupportedPanelRoute(client: any, attempts: any[]): Promise<any> {
...
function buildXrayConfigSnapshot(template: any) {
...
async function fetchPanelSettings(client: any): Promise<any> {
...
function resolvePayloadObject(payload: any): any {
...
function extractTemplateConfig(payload: any): any {
    if (!payload) return null;
...
export async function getXrayConfig(serverId: string): Promise<any> {
...
function ensureApiRuleFirst(rules: any[] = []): any[] {
...
function mergeRoutingSection(template: any, payload: any) {
...
function mergeOutboundsSection(_template: any, payload: any) {
...
function mergeDnsSection(template: any, payload: any) {
...
function mergeBalancersSection(template: any, payload: any) {
    if (!Array.isArray(payload)) {
...
function mergeReverseSection(_template: any, payload: any) {
    if (payload === null) return null;
    if (!payload || typeof payload !== 'object') {
...
function applySectionUpdate(template: any, section: string, payload: any) {
    if (section === 'template') {
        return payload;
    }
    const next = { ...template };
...
async function persistTemplate(client: any, source: string, rawSettings: any, nextTemplate: any): Promise<any> {
...
export async function updateXrayConfigSection(serverId: string, section: string, payload: any): Promise<any> {
    if (!SUPPORTED_WRITE_SECTIONS.has(section)) {
        const supported = Array.from(SUPPORTED_WRITE_SECTIONS).join(', ');
        const error: any = new Error(`Unsupported xray section: ${section} (supported: ${supported})`);
        error.status = 400;
        throw error;
    }
