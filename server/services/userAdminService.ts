...
function normalizeManagedRole(value: unknown): string {
...
function shouldFollowLoginEmail(target: any, payload: any = {}): boolean {
...
function buildSafeSubscriptionMigrationError(result: any) {
    const details = Array.isArray(result?.details) ? result.details : [];
    if (details.some((item: any) => item?.error === 'target-email-client-exists')) {
...
function resolveUpdateManagedUserArgs(actorOrDeps: any = 'admin', deps: any = {}) {
...
async function rollbackSubscriptionEmailTransition({
    ...
    tokenMigration = null,
}: any, deps: any = {}): Promise<{ ok: boolean; issues: string[] }> {
...
function emptyClientSyncResult() {
    return { total: 0, updated: 0, skipped: 0, failed: 0, details: [] as any[] };
}

function emptyDeploymentResult() {
    return { total: 0, created: 0, updated: 0, skipped: 0, failed: 0, details: [] as any[] };
}

function buildManagedUserEmailAliases(targetUser: any = null, primaryEmail: string = ''): string[] {
...
function applyInboundRetryScope(policy: any = null, targetInboundKeys: any = []) {
    ...
    const scopedKeys = currentKeys.length > 0
        ? keys.filter((key: string) => currentKeys.includes(key))
        : keys;
...
function resolveUserGroupForPolicy(targetUser: any = null, deps: any = {}) {
...
function resolveEffectivePolicyForUser(targetUser: any = null, rawPolicy: any = {}, deps: any = {}) {
...
function buildInboundScopeKey(serverId: string = '', inboundId: string = '') {
    return getInboundScopeKey(serverId, inboundId);
}

function isPolicyServerAllowed(policy: any = {}, serverId: string = ''): boolean {
    return isServerAllowedByPolicy(policy, serverId);
}

function isPolicyProtocolAllowed(policy: any = {}, protocol: string = ''): boolean {
    return isProtocolAllowedByPolicy(policy, protocol);
}

function isPolicyInboundBlocked(policy: any = {}, serverId: string = '', inboundId: string = ''): boolean {
    const key = buildInboundScopeKey(serverId, inboundId);
    if (!key) return true;
    return normalizeStringList(policy?.blockedInboundKeys).includes(key);
}

async function syncAddedInboundsToExistingUsers(addedInbounds: any[] = [], actor: string = 'admin', deps: any = {}): Promise<any> {
    ...
    const serverMap = new Map(allServers.map((item: any) => [String(item?.id || '').trim(), item]));
    const users = (Array.isArray(userRepo.list()) ? userRepo.list() : []).filter((item: any) => item?.role !== 'admin');
...
async function runManagedUserNodeSync(targetUser: any, actor: string = 'admin', options: any = {}, deps: any = {}): Promise<any> {
    const serverRepo = deps.serverRepository || serverRepository;
    const policyRepo = deps.userPolicyRepository || userPolicyRepository;
    ...
    const scopedServers = targetServerIds.length > 0
        ? allServers.filter((item: any) => targetServerIds.includes(String(item?.id || '').trim()))
        : allServers;
...
function flattenManagedUserSyncDetails(target: any, subscriptionEmail: string, enabled: boolean, group: any, stage: string) {
...
async function resyncManagedUserNodes(id: string, payload: any = {}, actor: string = 'admin', deps: any = {}): Promise<any> {
    const userRepo = deps.userRepository || userRepository;
...
function buildCsvCell(value: unknown): string {
    return `"${String(value || '').replace(/"/g, '""')}"`;
}

function stripSubscriptionAliasPath(user: any): any {
...
function listUsers(deps: any = {}) {
    ...
    return userRepo.list().map((item: any) => {
        const groupId = String(item?.groupId || '').trim();
        const group: any = groupId ? groups.get(groupId) : null;
...
function createManagedUser(payload: any = {}, deps: any = {}) {
    const userRepo = deps.userRepository || userRepository;
    const groupRepo = deps.userGroupRepository || userGroupRepository;
...
async function bulkSetUsersEnabled(payload: any = {}, actor: string = 'admin', deps: any = {}): Promise<any> {
...
function buildUsersCsv(deps: any = {}): string {
...
async function updateManagedUser(id: string, payload: any = {}, actorOrDeps: any = 'admin', deps: any = {}): Promise<any> {
    ...
    let policyMigration: any = {
        moved: false,
        fromEmail: currentSubscriptionEmail,
        toEmail: nextSubscriptionEmail,
        policy: null,
    };
    let tokenMigration: any = {
        moved: 0,
        fromEmail: currentSubscriptionEmail,
        toEmail: nextSubscriptionEmail,
        tokenIds: [],
...
async function updateUserSubscriptionBinding(id: string, payload: any = {}, actorOrDeps: any = 'admin', deps: any = {}): Promise<any> {
    if (!Object.prototype.hasOwnProperty.call(payload || {}, 'subscriptionEmail')) {
...
async function setManagedUserEnabled(id: string, payload: any = {}, actor: string = 'admin', deps: any = {}): Promise<any> {
    const userRepo = deps.userRepository || userRepository;
...
async function updateManagedUserExpiry(id: string, payload: any = {}, actor: string = 'admin', deps: any = {}): Promise<any> {
    const userRepo = deps.userRepository || userRepository;
    const policyRepo = deps.userPolicyRepository || userPolicyRepository;
    const target = userRepo.getById(id);
...
async function updateManagedUserPolicyByEmail(email: string, payload: any = {}, actor: string = 'admin', deps: any = {}): Promise<any> {
    const userRepo = deps.userRepository || userRepository;
    const policyRepo = deps.userPolicyRepository || userPolicyRepository;
    const serverRepo = deps.serverRepository || serverRepository;
    const toggleClients = deps.autoSetManagedClientsEnabled || autoSetManagedClientsEnabled;
...
async function provisionManagedUserSubscription(id: string, payload: any = {}, actor: string = 'admin', deps: any = {}): Promise<any> {
    const userRepo = deps.userRepository || userRepository;
...
function adminResetUserPassword(id: string, payload: any = {}, deps: any = {}) {
    const userRepo = deps.userRepository || userRepository;
...
async function deleteManagedUser(id: string, deps: any = {}): Promise<any> {
    const userRepo = deps.userRepository || userRepository;
    const policyRepo = deps.userPolicyRepository || userPolicyRepository;
    const tokenRepo = deps.subscriptionTokenRepository || subscriptionTokenRepository;
    const serverRepo = deps.serverRepository || serverRepository;
    ...
    const removedPolicyEmails: string[] = [];
    let cleanupError = '';
...
function summarizeGroupMembers(groupId: string = '', deps: any = {}) {
    const userRepo = deps.userRepository || userRepository;
    const normalizedGroupId = String(groupId || '').trim();
    return (Array.isArray(userRepo.list()) ? userRepo.list() : [])
        .filter((item: any) => item?.role !== 'admin' && String(item?.groupId || '').trim() === normalizedGroupId);
}

function normalizeUserIdList(input: any = []): string[] {
    return normalizeStringList(input);
}

function listUserGroups(deps: any = {}) {
    const groupRepo = deps.userGroupRepository || userGroupRepository;
    const userRepo = deps.userRepository || userRepository;
    const users = Array.isArray(userRepo.list()) ? userRepo.list() : [];
    return groupRepo.list().map((group: any) => {
        const members = users
            .filter((user: any) => user?.role !== 'admin' && String(user?.groupId || '').trim() === group.id)
            .map((user: any) => ({
                id: user.id,
...
function buildPolicyForGroupAssignment(currentPolicy: any = {}) {
...
function setUserGroupPolicyInheritance(targetUser: any, inheritGroup: boolean, actor: string = 'admin', deps: any = {}) {
    const policyRepo = deps.userPolicyRepository || userPolicyRepository;
...
function applyUserGroupMembership(groupId: string = '', memberUserIds: any = null, actor: string = 'admin', deps: any = {}) {
    const userRepo = deps.userRepository || userRepository;
    ...
    const userById = new Map(users.map((item: any) => [String(item?.id || '').trim(), item]));
    const invalidIds = desiredIds.filter((id: string) => !userById.has(id));
...
function createEmptyGroupSyncResult(group: any = null) {
    return {
        group,
        totalUsers: 0,
        syncedUsers: 0,
...
function addSyncGroupTotals(target: any, source: any) {
...
async function syncManagedUsersWithEffectivePolicies(members: any[] = [], actor: string = 'admin', deps: any = {}, detailDefaults: any = {}): Promise<any> {
...
async function createUserGroup(payload: any = {}, actor: string = 'admin', deps: any = {}): Promise<any> {
    const groupRepo = deps.userGroupRepository || userGroupRepository;
...
async function syncUserGroupMembers(groupId: string = '', actor: string = 'admin', deps: any = {}): Promise<any> {
    const groupRepo = deps.userGroupRepository || userGroupRepository;
    const group = groupRepo.getById(groupId);
    if (!group) {
...
async function updateUserGroup(id: string, payload: any = {}, actor: string = 'admin', deps: any = {}): Promise<any> {
    const groupRepo = deps.userGroupRepository || userGroupRepository;
    ...
    (sync as any).membership = membership;
    return {
        group: updated,
        membership,
        sync,
...
async function deleteUserGroup(id: string, actor: string = 'admin', deps: any = {}): Promise<any> {
    const groupRepo = deps.userGroupRepository || userGroupRepository;
    const userRepo = deps.userRepository || userRepository;
...
function buildUsersExportFilename(now: Date = new Date()): string {
