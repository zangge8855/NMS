...
function buildSubscriptionClientKeys(client: any = {}) {
...
function summarizeSubscriptionUsage(entries: any[] = [], policy: any = {}) {
    const summary = {
...
function buildSubscriptionUserInfoHeader(summary: any = {}) {
...
function normalizePolicyScopeMode(mode, selectedItems: any[] = []) {
...
function appendQuery(url, query: any = {}) {
...
function sortInboundsForServer(serverId, inbounds: any[] = []) {
    return systemSettingsStore.sortInboundList(serverId, inbounds);
}

function sortServersForSubscription(servers: any[] = []) {
...
function selectNativeSubIds(mode, allSubIds: any[] = [], fallbackSubIds: any[] = []) {
...
function buildXhttpExtra(xhttpSettings: any = {}) {
...
async function collectByServer(serverMeta, normalizedEmail, mode, options: any = {}) {
...
async function buildMergedLinksByEmail(email, options: any = {}) {
...
function applySharedTlsConfig(proxy, options: any = {}) {
...
function applyTransportToMihomoProxy(proxy, network, options: any = {}) {
...
function toMihomoHysteria2Proxy(outbound: any) {
...
function toMihomoTuicProxy(outbound: any) {
    if (!outbound) return null;
    const proxy: any = {
        name: outbound.tag,
        type: 'tuic',
...
function hasSurgeCompatibleLinks(links: any[] = []) {
...
function applyTransportToSingboxOutbound(outbound, network, options: any = {}) {
    const normalized = String(network || 'tcp').trim().toLowerCase() || 'tcp';
...
function applySharedSingboxTls(outbound, options: any = {}) {
    const security = String(options.security || '').trim().toLowerCase();
    const serverName = firstNonEmpty(options.servername, options.sni);
...
function buildSingboxConfigFromLinks(links: any[] = [], options: any = {}) {
    const config = buildSingboxConfigObject(links, options);
    if (!config) return '';
    return `${JSON.stringify(config, null, 2)}\n`;
}

function hasSingboxCompatibleLinks(links: any[] = []) {
...
function resolveSubscriptionConverterConfigUrls(options: any = {}) {
    const runtime = systemSettingsStore.getSubscription();
    return {
...
function buildExternalConverterUrl(baseUrl, format, sourceUrl, configUrl, options: any = {}) {
...
function buildSubscriptionUrls(publicBase, mode, serverId, options: any = {}) {
...
function getOrCreateScopeToken(email, serverId = '', actor = 'admin', options: any = {}) {
...
function appendSubscriptionAccessAudit(req, payload: any = {}) {
