...
interface SnapshotCache {
    ...
    promiseIncludeDetails: boolean;
    value: any;
    valueIncludeDetails: boolean;
    checkedAtMs: number;
}
...
const throughputBaselineByServerId = new Map<string, any>();
let warmLoopTimer: NodeJS.Timeout | null = null;
let warmLoopFollowupTimer: NodeJS.Timeout | null = null;

function safeTrafficTotal(value: unknown): number {
    const parsed = Number(value);
    return Number.isFinite(parsed) && parsed >= 0 ? parsed : 0;
}

function calculateMonotonicDelta(current: number, previous: number): number {
    if (!Number.isFinite(current) || !Number.isFinite(previous)) return 0;
    if (current < previous) return 0;
    return current - previous;
}

function resolveNetworkTrafficTotals(item: any = {}): { up: number; down: number } | null {
...
function buildThroughputSnapshot(current: any = {}, previous: any = null) {
...
function attachThroughputSnapshots(items: any[] = [], previousByServerId: Record<string, any> = {}) {
...
function normalizeReasonMessage(value: any): string {
    return String(
...
function classifyPanelError(error: any, options: { context?: string } = {}) {
    ...
    if ((Object.values(STATUS_REASON) as string[]).includes(reasonCode)) {
        return classified;
    }
    return {
...
function buildMaintenanceSnapshot(server: any) {
...
export async function collectServerStatusSnapshot(server: any, options: any = {}): Promise<any> {
    const checkedAt = new Date().toISOString();
    ...
    try {
        ...
        let inbounds: any[] = [];
        let onlineUsers: any[] = [];
        let panelSnapshot: any = null;
        const collectionIssues: any[] = [];
        if (includeDetails) {
            try {
...
export async function collectClusterStatusSnapshot(options: any = {}): Promise<any> {
...
export function getCachedClusterStatusSnapshot(): any {
    return snapshotCache.value;
}

export function resetClusterStatusSnapshotCache(): void {
    snapshotCache.promise = null;
    snapshotCache.promiseIncludeDetails = false;
...
export function evictServerStatusCaches(serverId: string): boolean {
    const normalizedServerId = String(serverId || '').trim();
    if (!normalizedServerId) return false;
    return throughputBaselineByServerId.delete(normalizedServerId);
}

function runWarmSnapshotCycle(options: any = {}) {
...
export function startClusterStatusWarmLoop(options: any = {}): void {
...
export function stopClusterStatusWarmLoop(): void {
    if (warmLoopTimer) {
