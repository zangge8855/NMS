const ALLOWED_PROTOCOLS = new Set<string>(['vmess', 'vless', 'trojan', 'shadowsocks']);
const POLICY_SCOPE_MODES = new Set<string>(['all', 'selected', 'none']);
const TRAFFIC_RESET_CYCLES = new Set<string>(['none', 'hourly', 'daily', 'weekly', 'monthly']);
const IP_LIMIT_POLICIES = new Set<string>(['first-wins', 'last-wins']);
...
function normalizeStringList(input: unknown = [], mapper: (x: any) => string = normalizeText): string[] {
    const result: string[] = [];
...
function normalizeScopeMode(value: unknown, selectedItems: unknown[] = [], fallback: string = 'all'): string {
...
function normalizeTrafficResetCycle(value: unknown, fallback: string = 'none'): string {
    const text = String(value || '').trim().toLowerCase();
    if (TRAFFIC_RESET_CYCLES.has(text)) return text;
    return TRAFFIC_RESET_CYCLES.has(fallback) ? fallback : 'none';
}

function normalizeIpLimitPolicy(value: unknown, fallback: string = 'first-wins'): string {
    const text = String(value || '').trim().toLowerCase();
...
export interface SanitizedPolicy {
    ...
    allowedInboundKeys: string[];
    blockedInboundKeys: string[];
    serverScopeMode: string;
    protocolScopeMode: string;
    expiryTime: number;
    limitIp: number;
...
function sanitizePolicy(input: any = {}, options: { defaults?: any } = {}): SanitizedPolicy {
...
function sanitizeUserPolicy(input: any = {}): any {
...
function sanitizeGroupPolicy(input: any = {}): any {
    return {
...
function getInboundScopeKey(serverId: string = '', inboundId: string = ''): string {
...
function isServerAllowedByPolicy(policy: any = {}, serverId: string = ''): boolean {
    const normalizedServerId = normalizeText(serverId);
...
function isProtocolAllowedByPolicy(policy: any = {}, protocol: string = ''): boolean {
...
function isInboundAllowedByPolicy(policy: any = {}, serverId: string = '', inboundId: string = ''): boolean {
...
function resolveEffectivePolicy(user: any = null, rawPolicy: any = {}, group: any = null): any {
    ...
    userPolicy.overrideFields.forEach((field: string) => {
