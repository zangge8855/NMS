...
function ipToBigInt(ip: string): { family: number; value: bigint | null } {
...
function parseCidrLine(line: unknown, label: string): CidrRecord | null {
...
function cidrContains(ipValue: bigint, family: number, record: CidrRecord): boolean {
...
function createDefaultFetcher() {
    return async ({ url, timeoutMs }: { url: string; timeoutMs: number }): Promise<string> => {
...
export interface IspSource {
    label: string;
    urls: string[];
}

export interface IpIspResolverOptions {
    enabled?: boolean;
    timeoutMs?: number;
    cacheTtlSeconds?: number;
    sources?: IspSource[];
    fetcher?: (params: { url: string; timeoutMs: number }) => Promise<string>;
}

export function createIpIspResolver(options: IpIspResolverOptions = {}) {
    const runtime = {
    ...
    const dataset = {
        records: [] as CidrRecord[],
        expiresAt: 0,
        loadedAt: 0,
        sourceCount: 0,
    };
    const ipCache = new Map<string, string>();
    let inflight: Promise<CidrRecord[]> | null = null;

    async function loadRecords(force: boolean = false): Promise<CidrRecord[]> {
        const now = Date.now();
    ...
    function resolveFromRecords(ip: unknown): string {
        const normalized = normalizeIpAddress(ip);
    ...
    async function lookup(ip: unknown): Promise<string> {
    ...
    function pickFromMap(map: unknown, ip: unknown): string {
        const normalized = normalizeIpAddress(ip);
    ...
    return {
        ...
        configure(next: Partial<IpIspResolverOptions> = {}) {
