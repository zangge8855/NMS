...
class AlertEngine {
     ...
     * 更新告警配置
     */
    configure(opts: Partial<AlertEngineConfig> = {}): void {
        if (opts.windowMs && opts.windowMs > 0) this.config.windowMs = opts.windowMs;
        if (opts.failureThreshold && opts.failureThreshold > 0) this.config.failureThreshold = opts.failureThreshold;
        if (opts.consecutiveThreshold && opts.consecutiveThreshold > 0) this.config.consecutiveThreshold = opts.consecutiveThreshold;
    }
     ...
     * 记录一次数据库写入失败
     */
    recordFailure(storeKey: string, errorMsg: string): void {
        const now = Date.now();
     ...
     * 获取当前窗口内的失败统计
     */
    getStats(): { windowMs: number; failuresInWindow: number; consecutiveFailures: number; lastSuccessAt: string | null } {
        this._cleanup();
        const failures = this.events.filter(e => e.type === 'write_failure');
        return {
