...
import notificationService, { SEVERITY, type SeverityType } from './notifications.js';

const DAY_MS = 24 * 60 * 60 * 1000;
const DEFAULT_INTERVAL_MS = 30 * 60 * 1000;

export interface ExpiryStageConfig {
    label: string;
    severity: SeverityType;
}

const STAGE_CONFIG: Record<string, ExpiryStageConfig> = {
...
function normalizeUserList(repo: any): any[] {
...
function formatDateTime(value: unknown): string {
    if (!value) return '暂无';
    try {
        return new Date(value as any).toLocaleString('zh-CN', { hour12: false });
    } catch {
        return String(value);
    }
}
...
export function resolveSubscriptionExpiryStage(expiryTime: unknown, now: number = Date.now()): ExpiryStageResult | null {
...
export interface SubscriptionExpirySnapshotItem extends ExpiryStageResult {
    username: string;
    email: string;
...
export interface SubscriptionExpirySnapshot {
    generatedAt: string;
    total: number;
    expiredCount: number;
    warningCount: number;
    items: SubscriptionExpirySnapshotItem[];
}

export function collectSubscriptionExpirySnapshot(options: any = {}): SubscriptionExpirySnapshot {
    ...
    const items: SubscriptionExpirySnapshotItem[] = [];
...
function buildExpiryNotification(item: any = {}): any {
    const identity = [
...
export function createSubscriptionExpiryNotifier(options: any = {}) {
    ...
    function clearResolvedUsers(activeKeys = new Set<string>()) {
    ...
    return {
        ...
        runOnce,
        resetForTests,
        collectSnapshot: (snapshotOptions: any = {}) => collectSubscriptionExpirySnapshot({
            now: now(),
            userStore: userRepo,
            policyStore: policyRepo,
            ...snapshotOptions,
