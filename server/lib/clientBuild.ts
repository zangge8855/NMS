...
export function normalizeClientBasePath(value: unknown, fallback: string = '/'): string {
...
export function shouldServeClientRequest(pathname: unknown = '/', basePath: unknown = '/'): boolean {
...
export function stripClientBasePath(pathname: unknown = '/', basePath: unknown = '/'): string | null {
    const normalizedPathname = String(pathname || '/').trim() || '/';
    const normalizedBasePath = normalizeClientBasePath(basePath, '/');
...
export function shouldServeCamouflageRequest(req: any = {}): boolean {
...
export function rewriteClientAssetPaths(html: string, basePath: unknown = '/'): string {
    const normalizedBasePath = normalizeClientBasePath(basePath, '/');
    const assetRoot = normalizedBasePath === '/' ? '' : normalizedBasePath;
    const assetPath = `${assetRoot}/assets/`;
    const logoPath = `${assetRoot}/nms-logo.png`;

    return html
        .replace(/((?:src|href)=["'])\/assets\//g, `$1${assetPath}`)
        .replace(/((?:src|href)=["'])\.\/assets\//g, `$1${assetPath}`)
        .replace(/((?:src|href)=["'])assets\//g, `$1${assetPath}`)
        .replace(/((?:href)=["'])\.\/nms-logo\.png/g, `$1${logoPath}`)
        .replace(/((?:href)=["'])nms-logo\.png/g, `$1${logoPath}`);
}

export function injectClientBasePath(html: string, basePath: unknown = '/'): string {
    const normalizedBasePath = normalizeClientBasePath(basePath, '/');
...
export interface ClientStaticHandlerOptions {
    clientBuild: string;
    getSiteAccessPath?: () => string;
    getSiteConfig?: (() => any) | null;
}
...
export interface ClientBuildFallbackHandlerOptions {
    clientIndexFile: string;
    hasClientIndex: (() => boolean) | boolean;
    missingBuildMessage?: string;
    getSiteAccessPath?: () => string;
    getSiteConfig?: (() => any) | null;
    readClientIndexFile?: (file: string) => string;
}
...
export function createClientBuildFallbackHandler({
    ...
    getSiteAccessPath = () => '/',
    getSiteConfig = null,
    readClientIndexFile = (file: string) => fs.readFileSync(file, 'utf8'),
}: ClientBuildFallbackHandlerOptions) {
...
export interface RegisterClientBuildRoutesOptions {
    rootDir?: string;
    missingBuildMessage?: string;
    getSiteAccessPath?: () => string;
    getSiteConfig?: (() => any) | null;
...
export function registerClientBuildRoutes(app: Express, options: RegisterClientBuildRoutesOptions = {}) {
    const {
        rootDir,
        missingBuildMessage = DEFAULT_MISSING_CLIENT_BUILD_MESSAGE,
        getSiteAccessPath = () => '/',
