...
function redactPathSegments(pathname: unknown): string {
    const parts = String(pathname || '').split('/');
    const publicIndex = parts.findIndex((part, index) => part === 'public' && parts[index - 1] === 'subscriptions');
    if (publicIndex < 0) return String(pathname || '');
...
export function redactRequestUrl(rawUrl: unknown = ''): string {
...
export function createSecurityHeadersMiddleware(options: SecurityHeadersOptions = {}) {
    ...
    const csp = [
        ...
        "connect-src 'self' ws://localhost:* wss://localhost:* ws://127.0.0.1:* wss://127.0.0.1:* https://ipwho.is",
    ].join('; ');
