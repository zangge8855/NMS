 ...
 */
export function checkPasswordComplexity(password: unknown): PasswordCheckResult {
 ...
 * @param password 待检查的密码
 */
export function checkPasswordNotUUID(password: unknown): PasswordCheckResult {
    const value = String(password || '');
    if (!value) {
        return { valid: false, reason: '密码不能为空' };
    }
 ...
 */
export function checkAccountPassword(password: unknown): PasswordCheckResult {
