...
function stringifyResidualDetails(value, omittedKeys: any[] = []) {
...
function buildDayKey(value: any = Date.now()) {
...
export function computeDailyBackupSchedule(options: any = {}) {
...
function sumTrafficTotals(entries: any[] = []) {
...
function formatHtmlDetailBlock(title, rows: any[] = [], { rawRows = false } = {}) {
...
function buildTelegramCommandMenu(items: any[] = []) {
...
function formatNotificationMessage(notification: any = {}) {
...
function formatAuditMessage(entry: any = {}, rule: any = {}) {
    const blocks = [];
...
function trimLines(lines: any[] = []) {
    return lines.filter(Boolean).join('\n').trim();
}

function appendSection(lines, title, rows: any[] = []) {
    const normalizedRows = rows.filter(Boolean);
...
function appendHtmlSection(blocks, title, rows: any[] = [], { rawRows = false } = {}) {
    const normalizedRows = rows.filter(Boolean);
    if (normalizedRows.length === 0) return;
    const icon = SECTION_ICONS.get(title) || '';
...
function joinHtmlMessage(title, blocks: any[] = [], options: any = {}) {
    const stdTitle = standardizeTelegramTitle(title);
...
function formatCommandRows(items: any[] = []) {
    return (Array.isArray(items) ? items : [])
        .map((item) => {
...
function formatCommandHtmlRows(items: any[] = []) {
    return (Array.isArray(items) ? items : [])
        .map((item) => {
            const command = String(item?.command || '').trim();
            const description = String(item?.description || '').trim();
...
function formatCommandHtmlTable(items: any[] = []) {
...
export function formatCommandCatalogMessage(options: any = {}) {
...
export async function collectStatusDigestContext(deps: any = {}) {
...
export function formatStatusDigestMessage(context: any = {}) {
...
function formatDetailBlock(title, rows: any[] = []) {
    const normalizedRows = rows.filter(Boolean);
...
function createAggregateBucket(key, payload: any = {}, windowMs = DEFAULT_DEDUP_WINDOW_MS) {
...
function updateAggregateBucket(bucket, payload: any = {}) {
...
function formatAggregateDigestMessage(bucket: any = {}) {
    const blocks = [];
    appendHtmlSection(blocks, '关键信息', [
...
function filterRecentItems(items: any[] = [], windowMs = 0, getTs?: any) {
...
export function createTelegramAlertService(options: any = {}) {
    ...
    function resolveSettings() {
        const stored: any = typeof options.getSettings === 'function'
            ? options.getSettings()
            : systemSettingsStore.getTelegram();
        const envDefaults: any = config.telegram || {};
    ...
    function noteNotificationOccurrence(payload: any = {}) {
    ...
    async function sendMessage(text, dedupKey = '', overrides: any = {}) {
        const settings = resolveSettings();
    ...
    async function sendBackupArchive(actor = 'system', options: any = {}) {
        const settings = resolveSettings();
        if (!settings.enabled || !settings.configured) {
    ...
    function scheduleNextDailyBackup(options: any = {}) {
        if (dailyBackupTimer) {
    ...
    async function sendCommandReply(text, chatId, kind = 'command', extras: any = {}) {
        await syncCommands();
        const overrides: any = {
            chatId,
            kind,
    ...
    function shouldSendNotification(notification: any = {}) {
        const settings = resolveSettings();
        if (!settings.enabled || !settings.configured) return false;
    ...
    function getAuditRule(entry: any = {}) {
        const settings = resolveSettings();
    ...
    async function notifyNotification(notification: any = {}) {
    ...
    async function notifySecurityAudit(entry: any = {}) {
        const settings = resolveSettings();
    ...
    async function callTelegramBotApi(method, body: any = {}, { requireConfiguredChat = true } = {}) {
        const settings = resolveSettings();
    ...
    async function callTelegramApi(method, body: any = {}) {
    ...
    async function dispatchCommand(command = '', { actor = 'telegram', chatId = '', positional = [], raw = '', page = 1 }: any = {}) {
    ...
    function schedulePeriodicDigests(options: any = {}) {
        if (opsDigestTimer) {
    ...
    function reloadSettings(options: any = {}) {
        commandsSynced = false;
        commandMenuCleared = false;
        commandSyncPromise = null;
        if (!running) return;
