...
export interface PanelAuthError extends Error {
    code?: string;
    isPanelAuthError: boolean;
    cause?: any;
}

export function createPanelAuthError(code: string, message: string, cause?: any): PanelAuthError {
    const error = new Error(message) as PanelAuthError;
    error.code = code;
...
export interface PanelCompatibilityError extends Error {
    code?: string;
    isPanelCompatibilityError: boolean;
    cause?: any;
}

function createPanelCompatibilityError(code: string, message: string, cause?: any): PanelCompatibilityError {
    const error = new Error(message) as PanelCompatibilityError;
    error.code = code;
...
export function normalizePanelErrorMessage(error: any, fallback: string = 'Panel request failed'): string {
    return String(
...
export function isUnsupportedPanelStatusError(error: any): boolean {
...
function isUnsupportedPanelCsrfError(error: any): boolean {
    const status = Number(error?.response?.status || 0);
    if ([404, 405, 410, 501].includes(status)) return true;
    const message = normalizePanelErrorMessage(error, '').toLowerCase();
    return message.includes('method not allowed')
...
function isSkippablePreLoginCsrfError(error: any): boolean {
    const status = Number(error?.response?.status || 0);
    return [401, 403].includes(status) || isUnsupportedPanelCsrfError(error);
}

function extractPanelCsrfToken(payload: any): string {
    const candidates = [
        payload?.obj,
        payload?.token,
        payload?.csrfToken,
...
function setPanelClientHeader(client: AxiosInstance, name: string, value: string): void {
    if (!client?.defaults?.headers) return;
    if (!value) {
        delete (client.defaults.headers as any)[name];
        if (client.defaults.headers.common) {
            delete client.defaults.headers.common[name];
        }
        return;
    }
    (client.defaults.headers as any)[name] = value;
    if (client.defaults.headers.common) {
        client.defaults.headers.common[name] = value;
    }
}

function setPanelApiToken(client: AxiosInstance, token: string): void {
...
async function refreshPanelCsrfToken(client: AxiosInstance, path: string, options: { ignoreUnsupported?: boolean; ignorePreLoginFailure?: boolean } = {}): Promise<string> {
    try {
...
export function derivePanelHealthFromStatus(statusPayload: any): {
    health: string;
    reasonCode: string;
...
export function classifyPanelError(error: any, options: { context?: string } = {}): {
    reasonCode: string;
    reasonMessage: string;
    retryable: boolean;
    health: string;
...
export async function fetchPanelServerStatus(client: AxiosInstance): Promise<any> {
...
export interface PanelClientWrapper {
 ...
 */
export async function createPanelClient(serverId: string, options: any = {}): Promise<PanelClientWrapper> {
...
async function ensurePanelApiTokenAuthenticated(client: AxiosInstance): Promise<AxiosInstance> {
    try {
 ...
 */
export async function ensureAuthenticated(serverId: string, options: any = {}): Promise<AxiosInstance> {
    ...
    try {
        ...
        const requestError: any = new Error(`Failed to authenticate with panel: ${panelMessage}`);
