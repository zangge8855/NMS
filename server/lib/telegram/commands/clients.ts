...
function formatTrafficUsage(helpers: any, user: any): string {
...
function summarizeUserLine(helpers: any, user: any): string {
...
export function registerClientCommands(registry: CommandRegistry, ctx: any): void {
    ...
    registry.register({
        ...
        handler: async ({ args }: { args: any }) => {
            ...
            const users = userAdmin.listUsers().filter((u: any) =>
                u.role !== 'admin'
                && (!keyword
            ...
            const lines = items.map((u: any) => summarizeUserLine(helpers, u));
    ...
    registry.register({
        ...
        handler: async ({ args }: { args: any }) => {
            ...
            const tokenLines = tokens.slice(0, 3).map((t: any) => {
