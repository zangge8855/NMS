...
function buildAuditRequest(actor: string, command: string) {
    return {
        user: {
...
async function recordTelegramAudit(services: any, { event, actor, command, target, outcome, details = {} }: {
    event: string;
    actor?: string;
    command: string;
    target?: any;
...
export function registerHighRiskCommands(registry: CommandRegistry, ctx: any, options: any = {}): void {
    ...
    function issueTokenFor(actor: string, operationKey: string) {
        return getRiskTokenStore().then((store: any) => store.issue({
    ...
    function consumeToken({ token, actor, operationKey }: { token: string; actor: string; operationKey: string }) {
        return getRiskTokenStore().then((store: any) => store.consume({
            token,
            actor: { userId: `telegram:${actor || 'tg'}`, role: 'admin' },
            operationKey,
        }));
        ...
        handler: async ({ args, ctx: invokeCtx }: { args: any; ctx: any }) => {
            ...
            return {
                ...
                pending: {
                    ...
                    execute: async ({ actor }: { actor?: string }) => {
