...
function buildAuditRequest(actor: string, command: string, extras: any = {}) {
    return {
        user: {
...
function summarizeClient(helpers: any, user: any): string {
    return `${helpers.escapeTelegramHtml(user.email || user.username || user.id)}`;
}

export function registerOpsCommands(registry: CommandRegistry, ctx: any): void {
            ...
            return {
                ...
                pending: {
                    ...
                    execute: async ({ actor }: { actor?: string }) => {
                        ...
                        const current: any = settingsStore.getSettings();
                        const patch = {
                            telegram: {
                                ...current.telegram,
    ...
    registry.register({
        ...
        level: 'write',
        summary: '解除告警静音',
        handler: async ({ ctx: invokeCtx }: { ctx: any }) => ({
            text: helpers.joinHtmlMessage('NMS Telegram 控制台', [
                `${helpers.sectionHeader('确认')}\n• 立即解除告警静音`,
            ], { subtitle: '解除静音' }),
            ...
            pending: {
                ...
                execute: async ({ actor }: { actor?: string }) => {
                    const settingsStore = (await import('../../../store/systemSettingsStore.js')).default;
                    const current: any = settingsStore.getAll();
