 ...
 * telegramAlertService.ts. This keeps the command modules testable in
 * isolation.
 */

export function escapeText(helpers: any, value: string = ''): string {
    return helpers.escapeTelegramHtml(value);
}

export function inlineCode(helpers: any, value: unknown = ''): string {
    return `<code>${helpers.escapeTelegramHtml(String(value ?? ''))}</code>`;
}

export function unknownCommand(helpers: any, hint: string = ''): string {
    return helpers.joinHtmlMessage('NMS Telegram 控制台', [
        `${helpers.sectionHeader('提示')}\n• ${hint || '未识别命令，请使用 <code>/help</code> 查看可用命令'}`,
    ], { subtitle: '命令入口' });
}

export function missingArgs(helpers: any, usageExample: string = ''): string {
    return helpers.joinHtmlMessage('NMS Telegram 控制台', [
        `${helpers.sectionHeader('提示')}\n• 参数不完整${usageExample ? `\n• 用法：${inlineCode(helpers, usageExample)}` : ''}`,
    ], { subtitle: '命令参数' });
}

export function notFound(helpers: any, what: string = '记录'): string {
    return helpers.joinHtmlMessage('NMS Telegram 控制台', [
        `${helpers.sectionHeader('提示')}\n• 未找到对应${what}`,
    ], { subtitle: '查询结果' });
}

export function findUserByIdOrEmail(users: any[] = [], needle: string = ''): any | null {
...
export function paginate<T>({ items = [], page = 1, pageSize = 8 }: { items: T[]; page?: number; pageSize?: number }): {
    items: T[];
    page: number;
    totalPages: number;
    total: number;
