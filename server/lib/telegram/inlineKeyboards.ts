...
function assertCallbackData(value: unknown): string {
...
export function buildConfirmKeyboard({ pendingId, confirmLabel = '✅ 确认', cancelLabel = '❌ 取消' }: {
    ...
    confirmLabel?: string;
    cancelLabel?: string;
} = { pendingId: '' }): InlineKeyboardMarkup {
...
export function buildMenuKeyboard(items: MenuItem[] = []): InlineKeyboardMarkup {
    const rows: InlineKeyboardButton[][] = [];
...
export interface ParsedCallbackData {
    kind: 'confirm' | 'cancel' | 'paginate' | 'menu' | 'unknown';
    id?: string;
    listKey?: string;
    page?: number;
    key?: string;
