...
export const CAMOUFLAGE_TEMPLATE_IDS = Object.freeze(['corporate', 'nginx', 'blog'] as const);
...
const CAMOUFLAGE_FILE_ASSET_SPECS: Record<string, Record<string, { publicPath: string; file: string; fallback?: string }>> = Object.freeze({
...
function collectClassTokens(): string[] {
    const tokens = new Set<string>(FALLBACK_CLASS_TOKENS);
...
const CLASS_MAP: Record<string, string> = Object.freeze(Object.fromEntries(
...
function normalizeTemplate(value: unknown, fallback: string = DEFAULT_TEMPLATE): string {
    const candidate = String(value || '').trim().toLowerCase();
    return (CAMOUFLAGE_TEMPLATE_IDS as readonly string[]).includes(candidate) ? candidate : fallback;
}

function normalizeTitle(value: unknown, fallback: string = DEFAULT_TITLE): string {
...
function renderTemplate(source: string, data: any): string {
...
export interface CamouflageAssetEntry {
    assetKey: string;
    templateName: string;
    absoluteFile: string;
    publicPath: string;
...
function buildCamouflageFileAssets(): { templates: Record<string, Record<string, CamouflageAssetEntry>>; routes: Map<string, CamouflageAssetEntry> } {
    const templates: Record<string, Record<string, CamouflageAssetEntry>> = {};
    const routes = new Map<string, CamouflageAssetEntry>();
    ...
    for (const [templateName, entries] of Object.entries(CAMOUFLAGE_FILE_ASSET_SPECS)) {
        ...
        for (const [assetKey, entry] of Object.entries(entries)) {
            ...
            const assetEntry: CamouflageAssetEntry = Object.freeze({
                assetKey,
                templateName,
                absoluteFile,
...
function createInlineAssets(title: string): Record<string, string> {
...
function createTemplateAssets(templateName: string, title: string): Record<string, string> {
...
function createRenderModel(options: RenderModelOptions = {}): any {
...
export function getCamouflageAssetPublicPath(templateName: string, assetKey: string): string {
    const templateAssets = CAMOUFLAGE_FILE_ASSETS.templates[normalizeTemplate(templateName)] || {};
    return templateAssets[assetKey]?.publicPath || '';
}

export function applyCamouflageResponseHeaders(res: Response): Response {
    res.setHeader('Cache-Control', CAMOUFLAGE_RESPONSE_CACHE_CONTROL);
    res.removeHeader('X-Powered-By');
    return res;
}

export interface CamouflageAssetMiddlewareOptions {
    getSiteConfig?: () => any;
}

export function createCamouflageAssetMiddleware({
    getSiteConfig = () => ({}),
}: CamouflageAssetMiddlewareOptions = {}) {
...
export function createSiteCamouflageHtml(options: RenderModelOptions = {}): string {
