...
export function normalizeAuditIp(value: unknown): string {
...
export function normalizeAuditUserAgent(value: unknown): string {
    const text = String(value || '').trim();
    if (!text) return '';
...
function configureResolvers(deps: any = {}) {
...
export async function enrichAuditEvents(items: any[] = [], deps: any = {}): Promise<any[]> {
    ...
    let ispMap = new Map<string, string>();
    if (geoEnabled) {
        try {
...
export async function enrichAuditEvent(item: any, deps: any = {}): Promise<any> {
