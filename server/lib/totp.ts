...
function base32Decode(input: unknown): Buffer {
...
function hotp(secretBuffer: Buffer, counter: number, digits: number = DEFAULT_DIGITS): string {
...
function counterForTime(unixSeconds: number, stepSeconds: number = DEFAULT_STEP_SECONDS): number {
...
export function generateTotp(secretBase32: string, atTimestampMs: number = Date.now(), options: TotpOptions = {}): string {
...
export function verifyTotp(token: unknown, secretBase32: string, atTimestampMs: number = Date.now(), options: TotpOptions = {}): boolean {
...
export interface BuildTotpUriOptions {
    secret: string;
    accountName: string;
    issuer?: string;
    digits?: number;
...
export function verifyBackupCode(code: unknown, hashedCodes: string[] = []): { match: boolean; remaining: string[] } {
