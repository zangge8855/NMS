...
export function resolveDataDir(value: unknown, rootDir: string = REPO_ROOT): string {
...
export function loadRuntimeEnv(envFile: string = DEFAULT_ENV_FILE): dotenv.DotenvConfigOutput {
    return dotenv.config({ path: envFile });
}

export function shouldServeClientBuild(env: Record<string, any> = process.env): boolean {
...
export function isWeakJwtSecret(value: unknown): boolean {
    const text = String(value || '');
    return text.length < 32 || text === DEFAULT_JWT_SECRET;
}

export function isWeakAdminPassword(value: unknown): boolean {
    const text = String(value || '');
...
export function isWeakAdminUsername(value: unknown): boolean {
    const text = String(value || '').trim().toLowerCase();
    if (!text) return true;
...
export function isWeakCredentialsSecret(value: unknown, jwtSecret: unknown): boolean {
    const text = String(value || '');
...
export interface StartupIssuesOptions {
    rootDir?: string;
    envFile?: string;
    includeClientBuildCheck?: boolean;
    includeEnvFileNotice?: boolean;
}

export function collectStartupIssues(env: Record<string, any> = process.env, options: StartupIssuesOptions = {}): SecurityIssue[] {
...
export function isVolatileDataDir(dataDir: string): boolean {
...
export function collectRuntimeDataIssues(dataDir: string): SecurityIssue[] {
    const issues: SecurityIssue[] = [];
...
export function ensureRuntimeDirectories(env: Record<string, any> = process.env, options: { rootDir?: string; logsDir?: string } = {}): { logsDir: string; dataDir: string } {
    const rootDir = resolve(options.rootDir || REPO_ROOT);
...
export function formatStartupIssues(issues: SecurityIssue[], options: { rootDir?: string; envFile?: string } = {}): string {
    const rootDir = resolve(options.rootDir || REPO_ROOT);
    const envFile = resolve(options.envFile || resolve(rootDir, '.env'));
    const lines = [
