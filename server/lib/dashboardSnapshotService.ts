...
const cpuHistoryCache = new Map<string, any>();
...
function evictServerDashboardCaches(serverId: string): boolean {
...
function getClientIdentifier(client: any = {}, protocol: string = ''): string {
...
function mergeInboundClientStats(inbound: any = {}): any[] {
    ...
    const statsRows: any[] = statsCandidates.find((item) => Array.isArray(item)) || [];
...
function normalizeOnlineValue(value: unknown): string {
    return String(value || '').trim().toLowerCase();
}

function normalizeOnlineKeys(item: any): string[] {
...
function buildClientOnlineKeys(client: any, protocol: string): string[] {
    const keys = new Set<string>();
...
function collectServerInboundRemarks(inbounds: any[] = [], serverName: string = ''): string[] {
    const enabledRemarks: string[] = [];
    const fallbackRemarks: string[] = [];
    const seen = new Set<string>();
    const normalizedServerName = String(serverName || '').trim().toLowerCase();

    const pushRemark = (list: string[], remark: unknown) => {
...
function withServerRemarkMeta(serverData: any, serverName: string = '') {
...
function buildManagedOnlineSummary(users: any[], serverPayloads: any[] = [], activeTrafficKeys = new Set<string>()) {
    const clientsMap = new Map<string, any>();
    const onlineMap = new Map<string, number>();
    const onlineServerIdMap = new Map<string, Set<string>>();
    const onlineServerLabelMap = new Map<string, Set<string>>();
    const managedEmailSet = new Set<string>();
    const serverTrafficByServerId: Record<string, any> = {};
    const emailMatchedOnlineSessions = new Map<string, Set<string>>();

    const rawTotalOnlineUsers = new Set<string>();
    ...
    serverPayloads.forEach((payload) => {
        ...
        const onlineMatchMap = new Map<string, Set<number>>();
            ...
            clients.forEach((client) => {
                ...
                if (!clientsMap.has(email)) {
                    clientsMap.set(email, {
                        ...
                        nodeLabels: new Set<string>(),
                    });
                }
                const entry = clientsMap.get(email)!;
                const matchedOnlineEntries = new Set<any>();
    ...
    const rows = (Array.isArray(users) ? users : [])
        ...
        .map((user) => {
            ...
            if (clientData.nodeLabels) {
                clientData.nodeLabels.forEach((n: string) => {
                    if (n) unionNodes.add(n);
                });
            }
            matchedServers.forEach((s: any) => {
    ...
    const serverOnlineUserCountByServerId = onlineRows.reduce((acc: Record<string, number>, row) => {
...
function buildDashboardPresenceFromPanelSnapshots(users: any[], panelSnapshots: any[] = [], activeTrafficEmails = new Set<string>()) {
...
function buildLiveServerPayloads(panelSnapshots: any[] = []) {
...
function toIsoTimestampMs(value: unknown): number {
    const timestamp = Date.parse(String(value || '').trim());
    return Number.isFinite(timestamp) ? timestamp : 0;
}

function sumTrafficTotals(items: any[] = []) {
...
function calculateMonotonicTrafficDelta(currentValue: unknown, previousValue: unknown): number {
...
function mergeTrafficTotals(base: any = {}, delta: any = {}) {
...
function shouldOverlayRealtimeTraffic(overview: any = {}): boolean {
...
function buildDashboardTrafficWindowTotal(options: any = {}, deps: any = {}) {
    ...
    const buildWindowValue = (totals: any = {}, ready = hasWindowSamples) => ({
...
function readDashboardTrafficWindowOverviews(trafficStatsStoreRef: any, requests: any[] = [], deps: any = {}) {
...
function buildDashboardTrafficWindowTotals(deps: any = {}) {
...
async function getServerCpuHistory(serverId: string, options: any = {}, deps: any = {}): Promise<any[]> {
    ...
    const request = ensureAuthenticatedRef(normalizedServerId)
        .then((client: any) => client.get('/panel/api/server/cpuHistory/30'))
        .then((res: any) => {
            const value = Array.isArray(res?.data?.obj)
                ? res.data.obj.map((point: any, index: number) => ({
                    time: index,
...
async function buildGlobalDashboardSnapshot(options: any = {}, deps: any = {}): Promise<any> {
    ...
    const enabledManagedEmailsGlobal = new Set<string>(
        (Array.isArray(users) ? users : [])
    ...
    Object.values(trafficStatsStoreRef.counters || {}).forEach((counter: any) => {
    ...
    const panelByServerId = Object.fromEntries(panelSnapshots.map((item: any) => [String(item?.server?.id || '').trim(), item]));
    const presence = buildManagedOnlineSummary(
        users,
        panelSnapshots.map((item: any) => ({
    ...
    const serverStatuses = servers.reduce((acc: Record<string, any>, server: any) => {
        ...
        const serverUniqueOnlineUsers = new Set<string>();
        rawOnlines.forEach((entry: any) => {
            const email = normalizeEmail(
                entry?.email || entry?.user || entry?.username || entry?.clientEmail || entry?.client || entry?.remark || entry
            );
            const rawIdentifier = email || String(entry?.email || entry?.id || entry?.user || entry?.username || entry?.clientEmail || entry?.client || entry || '').trim();
        ...
        acc[serverId] = withServerRemarkMeta({
            ...
            activeInbounds: Number(clusterItem?.activeInbounds || rawInbounds.filter((item: any) => item?.enable !== false).length || 0),
...
async function buildSingleDashboardSnapshot(serverId: string, options: any = {}, deps: any = {}): Promise<any> {
    const serverStoreRef = deps.serverStore || serverStore;
    const userStoreRef = deps.userStore || userStore;
    ...
    const clusterItem = Array.isArray(clusterSnapshot?.items)
        ? clusterSnapshot.items.find((item: any) => String(item?.serverId || '').trim() === normalizedServerId) || null
        : null;
    ...
    const activeSinceTs = Date.now() - 3 * 60 * 1000;
    const enabledManagedEmailsServer = new Set<string>(
        (Array.isArray(users) ? users : [])
            .filter((user) => user?.role !== 'admin' && user?.enabled !== false)
            .flatMap((user) => [normalizeEmail(user?.subscriptionEmail), normalizeEmail(user?.email)])
            .filter(Boolean)
