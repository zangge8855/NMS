...
export default function Inbounds() {
    ...
    const visibleOnlineUserCount = filteredInbounds.reduce((total, item) => total + safeNumber(item?.onlineUserCount), 0);
    const visibleOnlineSessionCount = filteredInbounds.reduce((total, item) => total + safeNumber(item?.onlineSessionCount), 0);
    const enabledVisibleCount = filteredInbounds.filter((item) => item.enable !== false).length;
    const scopeName = filterServerId === 'all'
        ? t('pages.inbounds.toolbarAllNodes')
        : (orderedServerOptions.find((item) => item.id === filterServerId)?.name || filterServerId);
    const toolbarCards = [
        {
            key: 'rules',
            label: t('pages.inbounds.rulesLabel'),
            value: String(filteredInbounds.length),
            detail: t('pages.inbounds.rulesDetail', { enabled: enabledVisibleCount }),
        },
        {
            key: 'online',
            label: t('pages.inbounds.onlineUsersLabel'),
            value: String(visibleOnlineUserCount),
            detail: t('pages.inbounds.onlineUsersDetail', { sessions: visibleOnlineSessionCount }),
        },
        {
            key: 'scope',
            label: t('pages.inbounds.scopeLabel'),
            value: scopeName,
            detail: isServerFilterLocked ? t('pages.inbounds.scopeLocked') : t('pages.inbounds.scopeManual'),
        },
    ];
                ...
                <div className="inbounds-toolbar glass-panel mb-6">
                    <div className="inbounds-toolbar-main">
                        <div className="inbounds-toolbar-copy">
                            <div className="inbounds-toolbar-kicker">{t('pages.inbounds.subtitle')}</div>
                            <div className="inbounds-toolbar-kpis">
                                {toolbarCards.map((card) => (
                                    <div key={card.key} className={`inbounds-summary-pill${card.key === 'scope' ? ' is-wide' : ''}`}>
                                        <span className="inbounds-summary-label">{card.label}</span>
                                        <span className="inbounds-summary-value">{card.value}</span>
                                        <span className="inbounds-summary-detail">{card.detail}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        <div className="inbounds-toolbar-filter-shell">
                            <select
                            ...
                            >
                                <option value="all">{t('pages.inbounds.allNodesOption', { count: servers.length })}</option>
                                {orderedServerOptions.map(s => (
                                    <option key={s.id} value={s.id}>{s.name}</option>
                                ))}
                            </select>
                            <span className={`inbounds-scope-chip${isServerFilterLocked ? ' is-locked' : ''}`}>
                                {isServerFilterLocked ? t('pages.inbounds.scopeLocked') : t('pages.inbounds.scopeManual')}
                            </span>
                        </div>
                    </div>
                    <div className="inbounds-toolbar-actions">
                        {selectedVisibleCount > 0 ? (
                            <div className="flex gap-2 items-center animate-fade-in inbounds-selection-bar">
                                <span className="text-sm font-bold px-2 text-primary">{t('pages.inbounds.selectedVisible', { count: selectedVisibleCount })}</span>
