...
function buildLogsSnapshotKey({ activeServerId, source }: { activeServerId?: string | number | null; source?: string }) {
    return `logs_viewer_v1:${String(activeServerId || '').trim() || 'none'}:${String(source || 'panel').trim() || 'panel'}`;
}

function readLogsSnapshot(snapshotKey: string) {
    const snapshot = readSessionSnapshot<any>(snapshotKey, {
...
function getSummaryToneMeta(tone?: string) {
    if (tone === 'danger') {
        return {
            badge: 'badge-danger',
...
function lineMatchesLevel(line: string, level?: string) {
...
function renderHighlightedLine(line: string, keyword?: string) {
...
function LogLine({ line, showServer, serverName, highlight, index }: LogLineProps) {
    const color = levelColor(line);
    const level = detectLogLevel(line);
    return (
        <div
            className={`log-line log-line-dynamic ${showServer && serverName ? 'has-server' : 'no-server'}`}
            style={{ '--log-line-color': color } as React.CSSProperties}
            data-level={level}
        >
...
function extractLogTimestamp(line: string) {
    const text = String(line || '');
...
export interface LogsProps {
    embedded?: boolean;
    sourceMode?: 'auto' | 'panel' | 'system' | 'xray' | string;
    displayLabel?: string;
}

export default function Logs({ embedded = false, sourceMode = 'auto', displayLabel = '' }: LogsProps) {
    ...
    // State
    const [logs, setLogs] = useState<LogEntry[]>(() => logsBootstrapRef.current?.logs || []);
    ...
    const [fetchSummary, setFetchSummary] = useState<any>(() => logsBootstrapRef.current?.fetchSummary || null);
    const [autoScrollEnabled, setAutoScrollEnabled] = useState(true);
    const [wrapLines, setWrapLines] = useState(true);
    const [immersiveMode, setImmersiveMode] = useState(false);

    // Global mode: server selection
    const [selectedServerIds, setSelectedServerIds] = useState<any[]>([]);
    ...
    const fetchSingleLogs = useCallback(async (options: { preserveCurrent?: boolean } = {}) => {
        ...
        try {
            ...
            setLogs(lines.map((line: string) => ({ line, serverName: activeServer?.name || '' })));
    ...
    }, [activeServerId, activeServer?.name, count, resolvedSource, sourceLabel, t, locale]);

    const fetchGlobalLogs = useCallback(async (options: { preserveCurrent?: boolean } = {}) => {
        const preserveCurrent = options.preserveCurrent === true;
        const requestId = logRequestIdRef.current + 1;
        logRequestIdRef.current = requestId;
        ...
        try {
            ...
            for (const item of items) {
                ...
                allLogs.push(...lines.map((line: string, index: number) => ({
                    line,
                    serverName,
            ...
            allLogs.sort((left, right) => {
                if (left.sortTs !== null && right.sortTs !== null && left.sortTs !== right.sortTs && left.sortTs !== undefined && right.sortTs !== undefined) {
    ...
    }, [isGlobal, selectedServerIds, servers, count, resolvedSource, sourceLabel, t, locale]);
    ...
    const toggleServer = (serverId: any) => {
        setSelectedServerIds((prev) =>
