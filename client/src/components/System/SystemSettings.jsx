...
function buildInviteEmailDraft() {
    return {
        emails: '',
        usageLimit: '1',
        subscriptionDays: '30',
...
export default function SystemSettings() {
    ...
    const [inviteEmailDraft, setInviteEmailDraft] = useState(buildInviteEmailDraft());
    const [inviteEmailTaskId, setInviteEmailTaskId] = useState(null);
    ...
    const savedSiteEntryPreview = useMemo(() => {
        if (typeof window === 'undefined') return savedSiteAccessPath;
        return `${window.location.origin}${buildAppEntryPath(savedSiteAccessPath, '/')}`;
    }, [savedSiteAccessPath]);
    ...
    const sendInviteEmails = async () => {
        if (!isAdmin) return;
        if (!inviteEmailDraft.emails.trim()) {
            toast.error('至少填写一个邀请邮箱');
            return;
        }

        const usageLimit = toBoundedInt(inviteEmailDraft.usageLimit, 1, 1, 1000);
        const subscriptionDays = toBoundedInt(inviteEmailDraft.subscriptionDays, 30, 0, 3650);
        setInviteCodeActionKey('send-email');
        try {
            const res = await api.post('/system/invite-codes/send-email', {
                emails: inviteEmailDraft.emails,
                usageLimit,
                subscriptionDays,
                message: inviteEmailDraft.message,
                entryUrl: savedSiteEntryPreview,
            });
            const payload = res.data?.obj || null;
            if (payload?.taskId) {
                setInviteEmailTaskId(payload.taskId);
            }
            setInviteEmailDraft((prev) => ({
                ...prev,
                emails: '',
            }));
            toast.success(res.data?.msg || '邀请邮件发送任务已创建');
        } catch (error) {
            toast.error(error.response?.data?.msg || error.message || '创建邀请邮件任务失败');
        }
        setInviteCodeActionKey('');
    };
                    ...
                    <div className="settings-invite-layout">
                        <div className="settings-access-stack">
                            ...
                            <div className="card p-3 settings-mini-card settings-detail-card">
                                <div className="settings-invite-card-head">
                                    <div className="settings-invite-card-title">生成邀请码</div>
                                    <div className="settings-invite-card-note">适合线下分发或批量备用，生成后会自动复制到剪贴板。</div>
                                </div>
                            ...
                            <div className="card p-3 settings-mini-card settings-detail-card">
                                <div className="settings-invite-card-head settings-invite-card-head--split">
                                    <div className="settings-invite-card-copy">
                                        <div className="settings-invite-card-title">邮件邀请注册</div>
                                        <div className="settings-invite-card-note">给指定邮箱单独发送注册链接和邀请码，适合正式发放。</div>
                                    </div>
                                    <span className={`badge ${emailStatus?.configured ? 'badge-success' : 'badge-neutral'}`}>
                                        {emailStatusLoading ? '检查中' : emailConfiguredLabel}
                                    </span>
                                </div>
                                <div className="settings-invite-highlight-list">
                                    <span className="settings-invite-highlight-chip">单邮箱单邀请码</span>
                                    <span className="settings-invite-highlight-chip">失败自动回收</span>
                                    <span className="settings-invite-highlight-chip">直达注册表单</span>
                                </div>
                                <div className="text-xs text-muted mb-3 settings-invite-status-note">
                                    {emailStatusLoading
                                        ? '正在检查 SMTP 状态...'
                                        : emailStatus?.configured
                                            ? '系统会为每个邮箱单独生成邀请码，并在发送失败时自动回收。'
                                            : 'SMTP 未配置，请先到“运维动作”里验证邮件链路。'}
                                </div>
                                <div className="settings-field-grid settings-field-grid--compact">
                                    <div className="form-group mb-0 settings-field-span-2">
                                        <label className="form-label" htmlFor="invite-email-targets">邀请邮箱</label>
                                        <textarea
                                            id="invite-email-targets"
                                            className="form-textarea settings-invite-textarea"
                                            rows={4}
                                            placeholder={'alice@example.com\nbob@example.com'}
                                            value={inviteEmailDraft.emails}
                                            onChange={(e) => setInviteEmailDraft((prev) => ({
                                                ...prev,
                                                emails: e.target.value,
                                            }))}
                                        />
                                        <div className="text-xs text-muted mt-1">支持换行、逗号或分号分隔多个邮箱。</div>
                                    </div>
                                    <div className="form-group mb-0 settings-field-span-2">
                                        <label className="form-label" htmlFor="invite-email-entry-url">注册入口链接</label>
                                        <input
                                            id="invite-email-entry-url"
                                            className="form-input settings-invite-readonly"
                                            type="text"
                                            value={savedSiteEntryPreview}
                                            readOnly
                                        />
                                        {savedSiteEntryPreview !== siteEntryPreview ? (
                                            <div className="text-xs text-muted mt-1">当前有未保存的站点入口修改，邀请邮件会继续使用已保存入口。</div>
                                        ) : null}
                                    </div>
                                    <div className="form-group mb-0">
                                        <label className="form-label" htmlFor="invite-email-usage-limit">每个邀请码可用次数</label>
                                        <input
                                            id="invite-email-usage-limit"
                                            className="form-input"
                                            type="number"
                                            min={1}
                                            max={1000}
                                            value={inviteEmailDraft.usageLimit}
                                            onChange={(e) => setInviteEmailDraft((prev) => ({
                                                ...prev,
                                                usageLimit: e.target.value,
                                            }))}
                                    ...
                                    <div className="form-group mb-0">
                                        <label className="form-label" htmlFor="invite-email-subscription-days">开通时长（天）</label>
                                        <input
                                            id="invite-email-subscription-days"
                                            className="form-input"
                                            type="number"
                                            min={0}
                                            max={3650}
                                    ...
                                    <div className="form-group mb-0 settings-field-span-2">
                                        <label className="form-label" htmlFor="invite-email-message">附加说明（可选）</label>
                                        <textarea
                                            id="invite-email-message"
                                            className="form-textarea settings-invite-textarea settings-invite-textarea--compact"
                                            rows={3}
                                            placeholder="例如：这是你的专属注册链接，请在 24 小时内完成注册。"
                                            value={inviteEmailDraft.message}
                                            onChange={(e) => setInviteEmailDraft((prev) => ({
                                                ...prev,
                                    ...
                                    <div className="form-group mb-0 settings-form-actions settings-invite-submit">
                                        <button
                                            className="btn btn-primary w-full"
                                            type="button"
                                            onClick={sendInviteEmails}
                                            disabled={inviteCodeActionLoading || emailStatusLoading || !emailStatus?.configured}
                                        >
                            ...
                            {latestInviteCodes.length > 0 ? (
                                <div className="card p-3 settings-mini-card settings-detail-card">
                                    <div className="settings-invite-card-head settings-invite-card-head--split">
                                        <div className="settings-invite-card-copy">
                                            <div className="settings-invite-card-title">本次生成的邀请码</div>
                                            <div className="settings-invite-card-note">邀请码明文只在创建时展示一次，请及时保存。</div>
                                        </div>
    ...
    return (
        <>
            ...
            />
            <TaskProgressModal
                taskId={inviteEmailTaskId}
                title="发送注册邀请邮件"
                onClose={() => {
                    setInviteEmailTaskId(null);
                    fetchInviteCodes({ quiet: true });
                    fetchEmailStatus({ quiet: true });
                }}
            />
