...
interface NodeHealthCopy {
    ...
    networkError: string;
    panelRequestFailed: string;
    inspectHint: string;
}

function getNodeHealthCopy(locale: string): NodeHealthCopy {
    return locale === 'en-US'
        ? {
...
function getNodeColor(serverData: any, t: (key: string, params?: Record<string, any>) => string): NodeColorInfo {
...
function getNodeFailureCopy(serverData: any, copy: NodeHealthCopy): string {
...
function buildSparkline(points: any[], width = 132, height = 34, padding = 3): SparklineData | null {
...
function getNodePriority(entry: any): number {
                    ...
                    <div className="node-health-tile-heading">
                        ...
                        {remarkPreview.length > 0 && (
                            <div className="node-health-tile-remarks" title={remarksTitle}>
                                {remarkPreview.map((remark: string) => (
            ...
            {isOnline ? (
                <div className="node-health-tile-meta">
                    ...
                    <div>
                        ...
                        <div className="node-health-tile-value">
                            {trafficReady && traffic !== null ? formatBytes(traffic) : '--'}
                        </div>
                    </div>
                </div>
            ) : (
...
export default function NodeHealthGrid({ servers, serverStatuses, trendHistory = {} }: NodeHealthGridProps) {
    const { t, locale } = useI18n();
    const navigate = useNavigate();
    const copy = getNodeHealthCopy(locale);
