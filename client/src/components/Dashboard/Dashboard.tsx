...
function buildInitialTrafficWindowTotals({ ready = false }: any = {}) {
    return {
...
function normalizeGlobalStatsSnapshot(stats: any = {}) {
    return {
...
function finiteNumberOrFallback(value: any, fallback = 0) {
    const parsed = Number(value);
    return Number.isFinite(parsed) ? parsed : fallback;
}

function normalizeStoredTrafficWindowTotals(windows: any = {}) {
    const normalizeWindow = (key: any) => {
...
function buildDashboardSnapshotServerStatuses(serverStatuses: any = {}) {
    return Object.entries(serverStatuses || {}).reduce((acc: any, [serverId, serverData]: [string, any]) => {
        acc[serverId] = {
...
function normalizeTrafficWindowPayload(windows: any = {}) {
    const readWindow = (...keys: any[]) => {
        const payload = keys
            .map((key) => windows?.[key])
            .find(Boolean) || {};
...
function buildTrafficWindowSubtitle(window: any = {}, t: any) {
...
export default function Dashboard() {
    ...
    const fetchWsTicket = useCallback(async ({ force = false }: any = {}) => {
        if (!token) {
            setWsTicket('');
            return;
        }
    ...
    const fetchSingleData = useCallback(async (options: any = {}) => {
    ...
    const fetchGlobalData = useCallback(async (options: any = {}) => {
        if (servers.length === 0) {
    ...
    const hydrateGlobalPresence = useCallback(async (options: any = {}) => {
                    ...
                    <div className="w-full dashboard-chart py-5">
                        {loading && cpuHistory.length === 0 ? (
                            ...
                            <Suspense fallback={<div className="skeleton w-full h-full opacity-10" />}>
                                <CpuHistoryChart cpuHistory={cpuHistory} endTick={cpuChartEndTick} t={t} />
                            </Suspense>
                        )}
                    </div>
                </div>
