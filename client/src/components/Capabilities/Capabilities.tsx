...
function buildCapabilitiesSnapshotKey(serverId: string | number): string {
    return `capabilities_view_v1:${String(serverId || '').trim()}`;
}

function readCapabilitiesSnapshot(serverId: string | number): any {
    const normalizedServerId = String(serverId || '').trim();
    if (!normalizedServerId) return null;

    const snapshot = readSessionSnapshot<any>(buildCapabilitiesSnapshotKey(normalizedServerId), {
...
function getCapabilitiesCopy(locale = 'zh-CN') {
    if (locale === 'en-US') {
        return {
            refresh: 'Refresh',
            toolbarSummary: (protocolCount: number, toolCount: number) => `Protocols ${protocolCount} · Tools ${toolCount}`,
            ...
            protocolCount: (count: number) => `${count} protocols`,
            noProtocols: 'No inbound protocols detected',
            legacyKeysLabel: (keys: string[]) => `Legacy aliases: ${keys.join(', ')}`,
            ...
            modulesDesc: (active: number, total: number) => `${active} of ${total} modules integrated`,
            toolsLabel: 'API Health Rate',
            toolsDesc: (active: number, total: number) => `${active} of ${total} endpoints active`,
            protocolsLabel: 'Active Protocols',
            protocolsDesc: (count: number) => `${count} protocol types online`,
        };
    }

    return {
        refresh: '刷新',
        toolbarSummary: (protocolCount: number, toolCount: number) => `协议 ${protocolCount} · 工具 ${toolCount}`,
        ...
        protocolCount: (count: number) => `${count} 种`,
        noProtocols: '未检测到入站协议',
        legacyKeysLabel: (keys: string[]) => `兼容旧命名: ${keys.join(', ')}`,
        ...
        modulesDesc: (active: number, total: number) => `已集成 ${active} / ${total} 个系统模块`,
        toolsLabel: 'API 接口探测率',
        toolsDesc: (active: number, total: number) => `已激活 ${active} / ${total} 个探测端点`,
        protocolsLabel: '运行中协议',
        protocolsDesc: (count: number) => `当前在线 ${count} 种协议类型`,
    };
}

function renderAvailability(value: boolean | undefined | null, copy: ReturnType<typeof getCapabilitiesCopy>) {
    if (value === true) {
        return (
...
function renderBooleanSupport(value: boolean, copy: ReturnType<typeof getCapabilitiesCopy>) {
    if (value) {
        return (
            <span className="cap-status-pill cap-status-pill--success">
                <HiOutlineCheckCircle className="text-sm" />
...
function renderAlignmentStatus(value: string | undefined, copy: ReturnType<typeof getCapabilitiesCopy>) {
    if (value === 'integrated') {
        return (
            <span className="cap-status-pill cap-status-pill--success">
                <HiOutlineCheckCircle className="text-sm" />
...
function renderProbeSource(source: string | undefined, copy: ReturnType<typeof getCapabilitiesCopy>) {
...
export default function Capabilities({ serverId = '', embedded = false }: CapabilitiesProps = {}) {
    ...
    const fetchCapabilities = async (options: { preserveCurrent?: boolean } = {}) => {
        if (!hasTargetServer) {
                ...
                {!data ? (
                    ...
                    <>
                            ...
                            <div className="capability-batch-grid">
                                <div className="capability-stack">
                                    ...
                                    <div className="flex gap-2 flex-wrap">
                                        {(data.batchActions?.clients || []).map((x: string) => (
                                ...
                                <div className="capability-stack">
                                    ...
                                    <div className="flex gap-2 flex-wrap">
                                        {(data.batchActions?.inbounds || []).map((x: string) => (
                        ...
                        <div className="card p-4">
                            <div className="flex gap-2 flex-wrap">
                                {(data.subscriptionModes || []).map((mode: string) => (
