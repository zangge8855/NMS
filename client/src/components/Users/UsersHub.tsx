...
import { normalizeEmail } from '../../utils/protocol';
import {
...
import { mergeInboundClientStats, resolveClientUsed, safeNumber } from '../../utils/inboundClients';
import { buildOnlineMatchMap, countClientOnlineSessions } from '../../utils/clientPresence';
import {
    applyManagedUsersSnapshot,
    fetchManagedUsers,
    getManagedUsersSnapshotMeta,
    invalidateManagedUsersCache,
} from '../../utils/managedUsersCache';
import { fetchServerPanelData, invalidateServerPanelDataCache } from '../../utils/serverPanelDataCache';
import { readSessionSnapshot, SESSION_SNAPSHOT_EVENT, writeSessionSnapshot } from '../../utils/sessionSnapshot';
import SubscriptionClientLinks from '../Subscriptions/SubscriptionClientLinks';
...
function hasVisibleUsersList(users = []) {
    return Array.isArray(users) && users.length > 0;
}

function createUsersStatsStateSnapshot({
    statsReady = false,
    clientsMap = new Map(),
    onlineMap = new Map(),
    inboundExpiries = [],
    ...
    return {
        statsReady: statsReady === true,
        clientsSize: clientsMap instanceof Map ? clientsMap.size : 0,
        onlineSize: onlineMap instanceof Map ? onlineMap.size : 0,
        inboundExpiryCount: Array.isArray(inboundExpiries) ? inboundExpiries.length : 0,
        inboundCount: Array.isArray(allInbounds) ? allInbounds.length : 0,
        partialErrorCount: Array.isArray(partialErrors) ? partialErrors.length : 0,
    };
}

function hasVisibleUsersStats(snapshot: any = {}) {
    return snapshot?.statsReady === true
        || Number(snapshot?.clientsSize || 0) > 0
        || Number(snapshot?.onlineSize || 0) > 0
        || Number(snapshot?.inboundExpiryCount || 0) > 0
        || Number(snapshot?.inboundCount || 0) > 0
...
function compareUsersFallback(a: any, b: any) {
    const order: Record<string, number> = { pending: 0, enabled: 1, active: 2, disabled: 3 };
...
export default function UsersHub() {
    ...
    const usersStateRef = useRef(Array.isArray(usersBootstrapRef.current) ? usersBootstrapRef.current : []);
    const liveUsersLoadedRef = useRef(initialUsersSnapshotMeta.source === 'live');
    const statsStateRef = useRef(createUsersStatsStateSnapshot({
        statsReady: statsBootstrapRef.current?.statsReady === true,
        clientsMap: new Map(statsBootstrapRef.current?.clientsEntries || []),
        onlineMap: new Map(statsBootstrapRef.current?.onlineEntries || []),
        inboundExpiries: statsBootstrapRef.current?.inboundExpiries || [],
        allInbounds: statsBootstrapRef.current?.allInbounds || [],
        partialErrors: statsBootstrapRef.current?.partialErrors || [],
    }));
    ...
    useEffect(() => {
        usersStateRef.current = Array.isArray(users) ? users : [];
    }, [users]);

    useEffect(() => {
        statsStateRef.current = createUsersStatsStateSnapshot({
            statsReady,
            clientsMap,
            onlineMap,
            inboundExpiries,
    ...
    }, [allInbounds, clientsMap, inboundExpiries, onlineMap, partialErrors, statsReady]);

    useEffect(() => {
        const handleSnapshotUpdate = (event) => {
            if (event?.detail?.source !== 'app-bootstrap' || event?.detail?.action !== 'write' || event?.detail?.key !== 'managed_users_v1') {
                return;
            }
            if (liveUsersLoadedRef.current) return;
            const snapshot = readSessionSnapshot('managed_users_v1', { fallback: null });
            if (!Array.isArray(snapshot)) return;
            const nextUsers = applyManagedUsersSnapshot(snapshot, { source: 'bootstrap' });
            usersBootstrapRef.current = nextUsers;
            usersStateRef.current = nextUsers;
            setUsers(nextUsers);
            setSelectedIds((previous) => new Set(
                Array.from(previous).filter((id) => nextUsers.some((user) => user.id === id))
            ));
            setPrimaryError('');
            setLoading(false);
        };
    ...
    const hydrateNodeStats = async (requestId: any, options: any = {}) => {
        const forceStats = options.forceStats === true;
        const preserveCurrentStats = hasVisibleUsersStats(statsStateRef.current);
        ...
        try {
            ...
            if (!hasVisibleUsersStats(statsStateRef.current)) {
                setClientsMap(new Map());
                setOnlineMap(new Map());
                setInboundExpiries([]);
                setAllInbounds([]);
    ...
    const fetchData = async (options: any = {}) => {
        ...
        const hasVisibleUsers = hasVisibleUsersList(usersStateRef.current);
        const hasVisibleStats = hasVisibleUsersStats(statsStateRef.current);
        ...
        try {
            const result = await fetchManagedUsers(api, {
                force: forceUsers,
                returnMeta: true,
            });
            if (requestId !== requestIdRef.current) return;
            const userList = Array.isArray(result?.users) ? result.users : [];
            liveUsersLoadedRef.current = String(result?.source || '').trim() === 'live';
            setUsers(userList);
            setSelectedIds((previous) => new Set(
            ...
            if (!hasVisibleUsersList(usersStateRef.current)) {
    ...
    const inboundByKey = useMemo(() => new Map<string, any>(
        allInbounds.map((inbound: any) => [String(inbound?.key || ''), inbound])
    ), [allInbounds]);
    ...
    const visibleGroupMemberCandidates = useMemo(() => {
        const query = String(groupMemberQuery || '').trim().toLowerCase();
        if (!query) return groupMemberCandidates;
        return groupMemberCandidates.filter((user) => [
            user.username,
            user.email,
            user.subscriptionEmail,
            user.groupName,
        ].some((value) => String(value || '').toLowerCase().includes(query)));
    }, [groupMemberCandidates, groupMemberQuery]);
    ...
    const openProvisionModal = async (user) => {
        ...
        try {
            ...
            setProvisionSpeedLimitUp(bytesPerSecondToKilobytesInput(policy.speedLimitUp));
            setProvisionSpeedLimitDown(bytesPerSecondToKilobytesInput(policy.speedLimitDown));
    ...
    const submitProvision = async (event) => {
        ...
        try {
            ...
            const res = await api.post(`/auth/users/${encodeURIComponent(provisionTargetUser.id)}/provision-subscription`, {
                ...
                speedLimitUp: kilobytesInputToBytesPerSecond(provisionSpeedLimitUp),
                speedLimitDown: kilobytesInputToBytesPerSecond(provisionSpeedLimitDown),
            ...
            if (policyEmail) {
                ...
                api.get(`/user-policy/${encodeURIComponent(policyEmail)}`)
                    .then((res) => {
                        ...
                        setEditSpeedLimitUp(bytesPerSecondToKilobytesInput(p.speedLimitUp));
                        setEditSpeedLimitDown(bytesPerSecondToKilobytesInput(p.speedLimitDown));
    ...
    const submitEdit = async (event) => {
        ...
        const payload: any = { username, email, subscriptionEmail, groupId: editGroupId };
        ...
        try {
            ...
            if (editGroupId && editInheritGroup) {
                ...
                if (kilobytesInputToBytesPerSecond(editSpeedLimitUp) !== Number(group?.speedLimitUp ?? 0)) overrideFieldsNext.add('speedLimitUp');
                if (kilobytesInputToBytesPerSecond(editSpeedLimitDown) !== Number(group?.speedLimitDown ?? 0)) overrideFieldsNext.add('speedLimitDown');
            ...
            if (policyEmail) {
                ...
                try {
                    const policyPayload = {
                        ...
                        speedLimitUp: kilobytesInputToBytesPerSecond(editSpeedLimitUp),
                        speedLimitDown: kilobytesInputToBytesPerSecond(editSpeedLimitDown),
    ...
    const formatInboundAssignmentLabel = (inboundKey) => {
        const inbound = inboundByKey.get(String(inboundKey || '').trim());
        if (!inbound) return String(inboundKey || '').trim();
        const remark = inbound.remark || inbound.protocol || inbound.inboundId;
        return `${inbound.serverName} / ${remark}`;
    };
    ...
    const openGroupModal = (group = null) => {
        ...
        if (group) {
            ...
            const memberIds = Array.isArray(group.memberIds) && group.memberIds.length > 0
                ? group.memberIds
                : users
                    .filter((user) => String(user?.groupId || '') === String(group.id || ''))
                    .map((user) => user.id);
    ...
    const toggleGroupMember = (userId, checked) => {
        toggleGroupArrayValue(setGroupMemberIds, userId, checked);
    };
                                ...
                                <div className="mb-3">
                                    ...
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <button
                                            ...
                                            onClick={() => setGroupMemberIds(Array.from(new Set([
                                                ...groupMemberIds,
