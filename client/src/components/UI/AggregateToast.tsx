...
export interface AggregateToastReport {
    total?: number;
    successCount?: number;
    failureCount?: number;
    failureItems?: Array<{ id: string; label?: string; message?: string; [key: string]: any }>;
    [key: string]: any;
}

export interface AggregateToastProps {
    toastId: string;
    visible?: boolean;
    title?: string;
    report?: AggregateToastReport | null;
    successLabel?: string;
    failureLabel?: string;
    detailsLabel?: string;
    retryLabel?: string;
...
function resolveDetailLines(report?: AggregateToastReport | null): Array<{ id: string; label?: string; message?: string; [key: string]: any }> {
    return Array.isArray(report?.failureItems) ? report.failureItems : [];
}

export function showAggregateToast(config: Partial<AggregateToastProps> = {}) {
    const duration = (config?.report?.failureCount ?? 0) > 0 ? 12000 : 7000;
