...
function getCopy(locale = 'zh-CN') {
    if (locale === 'en-US') {
        return {
            ...
            backupRemaining: (n: number) => `${n} backup codes left`,
            enabledAt: (ts: string) => `Enabled at ${ts}`,
            enable: 'Enable',
            disable: 'Disable',
    ...
    return {
        ...
        statusEnabled: '已启用',
        statusDisabled: '未启用',
        backupRemaining: (n: number) => `剩余 ${n} 个备用码`,
        enabledAt: (ts: string) => `启用时间: ${ts}`,
        enable: '开启',
        disable: '关闭',
...
export default function TwoFactorPanel({ locale = 'zh-CN' }: TwoFactorPanelProps) {
