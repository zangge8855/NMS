...
import { SESSION_SNAPSHOT_EVENT } from '../../utils/sessionSnapshot';
...
function isEditableShortcutTarget(target: any): boolean {
    if (!target || !(target instanceof Element)) return false;
    if (target.closest('[contenteditable="true"]')) return true;
    const tagName = String(target.tagName || '').toLowerCase();
    return tagName === 'input' || tagName === 'textarea' || tagName === 'select' || (target as any).isContentEditable === true;
}
...
function createRefreshCommand(copy: ReturnType<typeof buildCommandCopy>, pathname: string) {
    return {
        kind: 'command',
...
export default function Header({
    ...
    const [managedUsers, setManagedUsers] = useState<any[]>(() => getManagedUsersSnapshotMeta().users);
    ...
    const filteredItems = useMemo(() => {
        ...
        return searchableItems
            .map((item: any) => {
                ...
                if (keywords.some((keyword: string) => keyword.startsWith(query))) score += 3;
                if (description.includes(query)) score += 2;
                if (section.includes(query)) score += 1;
                return { ...item, score };
            })
            .filter(Boolean)
            .sort((a: any, b: any) => (b.score - a.score) || a.label.localeCompare(b.label, locale))
            .slice(0, MAX_SEARCH_RESULTS);
    }, [searchTerm, searchableItems, locale]);
    ...
    useEffect(() => {
        ...
        const handleClickOutside = (event: MouseEvent) => {
            const clickedInsideSearch = searchRef.current?.contains(event.target as Node);
    ...
    const computeSearchResultsPosition = useCallback(({ anchorRect, panelRect, viewport }: ComputePositionParams) => {
