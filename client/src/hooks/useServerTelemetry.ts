...
function buildTelemetrySnapshotKey(hours: number, points: number): string {
    return `server_telemetry_overview_v1:${hours}:${points}`;
}

function readTelemetrySnapshot(hours: number, points: number): TelemetryPayload | null {
    const snapshot = readSessionSnapshot<TelemetryPayload>(buildTelemetrySnapshotKey(hours, points), {
...
export interface UseServerTelemetryOptions {
    enabled?: boolean;
    hours?: number;
    points?: number;
...
export interface RefreshTelemetryOptions {
    quiet?: boolean;
    preserveCurrent?: boolean;
}

export interface UseServerTelemetryReturn {
    telemetry: TelemetryPayload;
    telemetryByServerId: Record<string, any>;
    telemetryLoading: boolean;
    refreshTelemetry: (fetchOptions?: RefreshTelemetryOptions) => Promise<TelemetryPayload>;
}

export default function useServerTelemetry(options: UseServerTelemetryOptions = {}): UseServerTelemetryReturn {
    ...
    const telemetryBootstrapRef = useRef<TelemetryPayload | null>(readTelemetrySnapshot(hours, points));
    const telemetryStateRef = useRef<TelemetryPayload>(telemetryBootstrapRef.current || EMPTY_TELEMETRY);
    const liveTelemetryLoadedRef = useRef(false);
    const [loading, setLoading] = useState(false);
    const [telemetry, setTelemetry] = useState<TelemetryPayload>(() => telemetryBootstrapRef.current || EMPTY_TELEMETRY);
    ...
    const refresh = useCallback(async (fetchOptions: RefreshTelemetryOptions = {}): Promise<TelemetryPayload> => {
        if (!enabled) {
