...
export interface NotificationContextType {
    ...
    ensureExpandedNotifications: () => Promise<void>;
    refresh: (options?: Record<string, any>) => Promise<any>;
}
...
function readNotificationSnapshot(): NotificationSnapshot | null {
    const snapshot = readSessionSnapshot<NotificationSnapshot>(NOTIFICATION_SNAPSHOT_KEY, {
...
export function NotificationProvider({ children, wsLastMessage, enabled = true }: NotificationProviderProps) {
    const bootstrapRef = useRef<NotificationSnapshot | null>(enabled ? readNotificationSnapshot() : null);
    const [notifications, setNotifications] = useState<NotificationItem[]>(() => (enabled ? (bootstrapRef.current?.notifications || []) : []));
    const [unreadCount, setUnreadCount] = useState<number>(() => (enabled ? (bootstrapRef.current?.unreadCount || 0) : 0));
    ...
    const pendingFetchesRef = useRef(new Map<string, Promise<any>>());
    const lastLoadedAtRef = useRef<{ preview: number; expanded: number }>({
    ...
    useEffect(() => {
        ...
        setUnreadCount((current) => (current !== 0 ? 0 : current));
        setLoading(false);
        clearSessionSnapshot(NOTIFICATION_SNAPSHOT_KEY);
    }, [enabled]);
    ...
    }, [enabled, notifications, unreadCount]);
    ...
    const fetchNotifications = useCallback(async (options: { limit?: number; force?: boolean; preserveCurrent?: boolean } = {}) => {
        if (!enabled) {
            return [];
        }
        ...
        const ageMs = Date.now() - Number(lastLoadedAtRef.current[cacheKey as 'preview' | 'expanded'] || 0);
        ...
        if (wsLastMessage.type === 'NMS_NOTIFICATION') {
            ...
            if (notification) {
                ...
                const toastFn = notification.severity === 'critical'
                    ...
                    ? (msg: any, opts: any) => toast(msg, { ...opts, icon: '⚠️' })
                    : toast;
    ...
    return (
        <NotificationContext.Provider value={{
            ...
            refresh: (options: Record<string, any> = {}) => fetchNotifications({
