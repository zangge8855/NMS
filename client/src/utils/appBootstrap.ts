import { writeSessionSnapshot, clearAllSessionSnapshots } from './sessionSnapshot';
import { invalidateManagedUsersCache } from './managedUsersCache';
import { invalidateServerPanelDataCache } from './serverPanelDataCache';
...
function buildTelemetrySnapshotKey(hours: number = 24, points: number = 24): string {
    return `server_telemetry_overview_v1:${Number(hours || 24)}:${Number(points || 24)}`;
}

export function applyAppBootstrapSnapshots(payload: Record<string, any> = {}): void {
    if (!payload || typeof payload !== 'object') return;
    const issuedAt = String(payload?.issuedAt || '').trim();
    const withIssuedAt = <T extends Record<string, any>>(value: T): T => (issuedAt ? { ...value, issuedAt } : value);
